...
★이 파일은 `설계 계약 문서` 의 구현체다.
  둘이 어긋나면 결함이다. 바꿀 때는 계약 문서를 먼저 고친다(작업 규칙 §3.5).
...
class Evidence(BaseModel):
    ...
    ★source_type · source_id · observed_at 은 의무다.
      근거 없는 문장을 답변에 넣지 않는다(설계 원칙 §0.1).
    """
...
class ContextPack(BaseModel):
    ...
    def _budget_and_signals(self) -> ContextPack:
        ...
        # ★신호 없는 축소는 폴백이다(작업 규칙 §3.2). degraded 면 무엇을 뺐는지 남긴다.
...
class ActionProposal(BaseModel):
    """Team 이 내는 제안. ★Team 은 side effect 를 실행하지 않는다(설계 원칙 §0.2).
