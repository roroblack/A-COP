...
class SqlGraphAdapter:
    ...
    #: ★버그사냥 2026-08-17 — DoD-21 이 추가한 세 축(has_issue/governed_by/
    #:   handled_by, `neighbors()` 의 51-67행 주석 참조)이 여기 빠져 있었다.
    #:   `neighbors()` 를 직접 그 세 타입으로 부르면 나오는데, `subgraph()`
    #:   는 자기만의 고정 목록을 써서 결과에서 조용히 빠졌다 — subgraph() 는
    #:   지금 프로덕션 호출부가 없어(grep 확인) 당장 뚫리진 않지만, "이 노드
    #:   주변 전체"를 낸다는 이름의 함수가 실제로는 절반만 냈다.
    EDGE_TYPES = ("owns", "has_event", "proposed", "approved", "contains", "in_document",
                 "has_issue", "governed_by", "handled_by")

    async def subgraph(self, root_id: str, depth: int = 2) -> dict:
        rows = await self.neighbors(root_id, list(self.EDGE_TYPES), depth)
