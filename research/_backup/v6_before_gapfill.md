...
아래 경로는 `final_project_sample/` 저장소를 **2026-08-13 실측**한 값이다.
`app/core/contracts.py`·`app/application/controller.py` 같은 평면 경로는 하위호환 shim이며
(`from app.core.case_runtime... import *`), 실제 구현은 중첩 경로에 있다. 수정은 중첩 경로에 한다.
