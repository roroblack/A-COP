...
def test_dataset_counts_and_allocation_and_disjointness():
    ...
    assert {k: sum(1 for x in golden if x["case_id"].startswith(k)) for k in ("g-billing", "g-technical", "g-feedback")} == {"g-billing":20,"g-technical":20,"g-feedback":20}
    assert {k: sum(1 for x in holdout if x["case_id"].startswith(k)) for k in ("h-billing", "h-technical", "h-feedback", "h-other")} == {"h-billing":5,"h-technical":5,"h-feedback":5,"h-other":5}
