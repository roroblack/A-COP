...
지금 활성 규칙 33개 중 24개에만 결함이 붙어 있다. 나머지 9개는 `CLAUDE.md` 가 못 박은
규칙인데 깨져도 아무 테스트가 울지 않는다 — 결함조차 없어서 게이트에도 안 잡힌다.
...
- 결함 카탈로그: 후보 39건 중 낼 수 있는 것 **38건**
  (1건은 신호가 실행마다 갈려 제외 — `defects_more3.py` 의 `excluded` 참고)
- 사각지대는 현재 0건이다. 회귀 테스트를 넣기 전에는 18건이었다 →
