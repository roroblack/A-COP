...
이 표가 오래됐으면 [`program/research/index.md`](program/research/index.md)의 「현재 기준 사실」 표가 정본이다. 둘은 같이 갱신한다. 세부 설계·운영 사실은 [`wiki/index.md`](wiki/index.md)와 각 저장소의 `wiki/`가 정본이다(2026-09-07 전환 완료). **읽는 순서는 wiki 먼저다.** `docs/`는 작업 기록(evidence·리포트)이라 wiki가 인용한 근거를 확인할 때 연다. 두 곳이 다르면 wiki를 고치거나, 기록이 낡았다는 주석을 붙인다 — 기록 자체는 고치지 않는다.
