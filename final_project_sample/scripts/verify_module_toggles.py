...
`final_project_cs` 의 같은 스크립트를 이 저장소 구조에 맞춰 옮긴 것이다.
화면 경로가 `/ops/**` 이고, 관리자 화면이 없어 graph 는 조립 함수로 직접 확인한다.
...
print('GET /ops/voc ->', client.get('/ops/voc').status_code)
print('상단 메뉴에 VOC 링크 ->', '/ops/voc' in client.get('/ops/cases').text)
"""
...
try:
    adapter = composition.build_graph_store(connection=None, tenant_id='demo')
    print('build_graph_store ->', type(adapter).__name__)
except Exception as exc:
    print('build_graph_store ->', type(exc).__name__, '|', exc)
"""
...
def main() -> int:
    ...
    run("graph_store 켬 — 어댑터가 만들어진다", "graph_store", True, PROBE_GRAPH)
    run("graph_store 끔 — 조립이 거부된다", "graph_store", False, PROBE_GRAPH)
