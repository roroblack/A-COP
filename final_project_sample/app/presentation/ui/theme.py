...
def page(title: str, body: str, *, current: str = "", lede: str = "",
         nav: Any = None, brand: str = "운영 콘솔") -> str:
    """★nav 를 받는다. 두 관객이 서로의 메뉴를 보면 안 된다 —
    고객사에게 Composer·DoD 가 보이면 우리 사정이 새고,
    개발 콘솔에 고객 Case 메뉴만 있으면 정작 할 일이 안 보인다."""
    links = "".join(
    ...
    return (
        ...
        f"<span class='brand'>A-COP<span>{esc(brand)}</span></span><nav>{links}</nav></div></div>"
