...
★**두 운영 방식(2026-08-30). 고르는 주체가 이 프로그램이다.**

| `CONSOLE_COMPOSER_MODE` | 무엇을 부르나 | 언제 |
|---|---|---|
| `direct`(기본) | 대상 제품에 함께 설치된 Composer 를 **직접** 부른다 | 대상이 적을 때 |
| `central` | **중앙 설정 서비스** 한 곳을 부르고 `CONSOLE_COMPOSER_DEPLOYMENT_ID` 로 대상을 지정한다 | 대상이 많을 때(수천 규모) |

요청·응답 모양은 같다 — 주소와 헤더(`X-Deployment-Id`) 하나만 다르다. 그래서
화면 코드는 바뀌지 않고 환경변수만 바꾸면 된다. 근거와 배경은
`program/plan/A-COP_Composer_중앙설정저장소_결정.md`.

★**알 수 없는 mode 값은 거부한다**(기본값으로 삼키지 않는다). 오타를 삼키면
"중앙으로 켠 줄 알았는데 대상을 직접 건드리고 있었다" 가 되고, 그건 남의
설정을 바꾸는 사고다. 같은 이유로 **화면 맨 위에 지금 어느 방식인지, central
이면 어느 대상인지 표시한다** — 이 화면의 [적용] 이 누구 설정을 바꾸는지
보여야 한다.
...
- ★**계약 1.1(2026-08-31)부터 실행 중인 조립과 저장된 선언이 갈린다.**
  `active_revision`(지금 돌고 있는 것) · `desired_revision`(저장된 것) ·
  `reload_state`. 저장에 성공해도 대상이 반영하기 전까지는 옛 조립이 돈다 —
  화면은 그걸 **맨 위에서 말한다**(`_reload_state_note`). 1.0 대상은 이 필드가
  없으므로 **아무 말도 하지 않는다**(없는 것을 "정상" 으로 적지 않는다).
  기본 허용 버전은 `1.0,1.1` 이다
- ★**[반영] 버튼은 §0.3 예외에 하나 더 얹힌 호출이다.** `POST /admin/reload`
  (대상 scope `ops:reload`, `CONSOLE_RELOAD_TOKEN`)를 부른다 — 대상이 자기
  프로세스 안에서 하는 일을 부를 뿐, 여기서 대상 파일·DB·파이썬을 건드리지
  않는다. 주소는 `introspection_url` 에서 만든다(따로 받으면 **남의 프로세스를
  반영시키는** 사고가 난다). 어긋났을 때만 내고, 토큰이 없으면 버튼 대신
  **왜 없는지**를 적는다
