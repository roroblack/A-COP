...
이 문서는 v6 전체를 계승한 새 기준선이다. `A-COP_구현계획서_v6.md`는 보존본이며 수정하지 않는다. `A-COP_구현계획서_v5.md`, `A-COP_구현계획서(4).md`, `A-COP_구현계획서_A2A_Graph반영.md`도 보존·수정 금지다.
...
| 이후 기준선 | v7이 이후 구현·평가·심사의 기준선이다. v6와 이전 버전은 보존본이다. |
...
```text
   ...
   Billing / Technical / VOC     재고·가격·발주·배송·정산
       (10주 구현)                    (vision)
```
...
상세 설계는 `research/20260816_제품구조_런타임과_도메인팩.md` 참조.
...
VOC/Feedback Analytics도 Agent Team으로 등록한다. VOC는 이상징후 탐지, 반복 불만 식별, 공급처·처리 성능 감시라는 고유 책임을 갖고, 다른 Team에 Task를 위임하거나 알림을 보낼 수 있는 고유 권한을 갖는다. 집계·시계열·임계값 정책이라는 고유 지식과 집계·관측·알림 Tool이라는 고유 경계도 있다. 무엇보다 “이것이 급증인가”, “누구에게 알릴 것인가”를 판단한다. Agentic Controller가 전역 조정 책임을 갖는 것과 달리 VOC는 업무 판단을 수행하는 Team이다.
...
└───────────────┬───────────────────────────────┬─────────────────┘
       ...
       RAG · DB facts · history · graph     Billing / Technical / VOC
                │                               │
                │                               │ ActionProposal
                │                               ▼
                │                    Controller → Core 2
...
필요 Team과 착수 Team을 구분한다. 기능상 필요 Team은 Catalog & Verification, Procurement, Order & Payment, Fulfillment & Logistics, Return & Refund, VOC/Feedback Analytics다. 10주 착수 구현은 Catalog & Verification, Procurement+Order & Payment 통합, Fulfillment & Logistics, VOC 배치/Team 계약을 우선한다. Return & Refund는 Registry 계약과 Mock으로 남기고, Catalog & Verification의 A2A Remote Team은 별도 프로세스 fixture로 실제 왕복을 구현한다. **Registry 등록은 실제 구현을 의미하지 않는다.** 등록된 capability가 있어도 실행 가능한 module·fixture·contract test가 없으면 구현 완료로 세지 않는다.
...
구독 해지 후 결제는 Billing Team이 Context를 읽고 환불 `ActionProposal`을 반환한다. 권한 동기화 오류는 Technical Team이 상태와 정책 근거를 비교한다. 반복 불만은 VOC/Feedback Analytics Team이 Case event와 일일 집계를 사용해 alert와 위임 제안을 만든다.
...
1단계는 도메인·Case·ERD·상태·Contract를 확정한다. 2단계는 Core Basement MVP, Registry, 세 Port, Context Broker, Shared State를 구현한다. 3단계는 Billing/Technical/VOC Team과 RAG를 구현한다. 4단계는 REST/MCP/A2A, 5단계는 UI와 trace/approval, 6단계는 평가와 고도화를 수행한다.
...
착수 시점에는 Billing/Subscription · Technical Entitlement · VOC/Feedback Analytics 축으로 나눈다.
...
| 2 | CAS·transition·Registry | REST/MCP skeleton | Billing/Technical/VOC contract, Catalog A2A Card | harness skeleton |
...
| 6 | Shared State merge | Catalog Verification 더미 A2A adapter | VOC 위임 제안·`ActionProposal` 흐름 | end-to-end demo |
| 7 | GraphStorePort·SQL adapter | MCP/A2A 보안 | 관계 질의용 fixture | Graph gate 측정 |
| 8 | 재처리·경합 테스트 | input-required·callback·cancel | LOCAL/A2A 동일 결과, VOC 배치/Team | 60건 평가 실행 |
...
|---|---|---|---|---|---|
| 6 | A-COP Lab *(예시)* | [팀원명 입력] | 다중 에이전트 서빙 기반 고객 피드백 분석 및 맞춤형 응대 자동화 시스템 | **기업이 자사 고객 응대와 운영 업무를 Agentic AI로 감독하게 하는 B2B 플랫폼.** 하나의 Runtime이 상태 있는 Case, 업무별 Team, 승인·idempotency·감사·평가를 공통 제공하고, CS Pack(Billing/Technical/VOC)과 Commerce Ops Pack(재고·가격·발주·배송·정산)을 Registry로 교체·확장한다. Team은 side effect를 직접 실행하지 않고 제안만 반환하며, 모든 판단은 근거와 함께 기록되어 재생·감사할 수 있고 golden/holdout과 통계 검정으로 측정한다. 소비자는 기존 자기 Agent로 **Commerce MCP와 CS MCP**를 통해 상품·주문·배송·문의까지 이용하고, 독립 Agent System은 A2A로 업무를 위임받는다. 운영자는 **human-on-the-loop**로 이상 신호와 고위험 Action만 확인한다. 자체 호스팅이 가능해 데이터가 외부로 나가지 않는다. 10주에는 CS와 제한된 검증 쇼핑몰을 구현하고, 사이트 생성·판매와 전면 Commerce Ops 자동화는 vision이다. | **MVP 필수**: Python, FastAPI, React, PostgreSQL, pgvector, LangGraph, RAG, MCP, A2A, API Key+Scope<br>**Phase 2 선택**: OAuth2/OIDC, Redis/RabbitMQ, AgeGraphAdapter·Neo4jGraphAdapter, Docker |
