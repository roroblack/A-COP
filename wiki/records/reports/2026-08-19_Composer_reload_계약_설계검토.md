...
> ★**후속(2026-08-31) — 아래 결론은 더 이상 현재 상태가 아니다.**
> §5 의 재검토 트리거 첫 항목("대상과 Composer 가 동일한 durable config store 를
> 쓰기로 결정된 때")이 중앙 설정 저장소 결정(2026-08-30)으로 충족됐다. 그래서
> **후보 2(관리자 신호)를 채택**했다 — `POST /admin/reload`(scope `ops:reload`).
> 이 문서의 분석과 요구사항(§3 의 active/desired/reload_state 구분, 실패 시 옛
> 조립 보존)은 그대로 구현의 계약이 됐다.
> 구현·실측: `wiki/records/reports/2026-08-31_reload_계약_구현.md`
