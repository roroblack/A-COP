...
등록한다. `wiki/records/handoff/13`(Composer 쓰기채널 계약)은 "VPN/SSH 터널 + 단명
...
1. **`wiki/records/handoff/13_Composer_쓰기채널_계약.md`** — "경계 / 운영 제한"
...
2. **`infra/aws/main.tf`**(또는 관련 `.tf` 파일) — ALB ingress 를
   ...
   `wiki/records/plans/2026-08-17_Docker_AWS_배포_모듈화_계획.md` 참고) —
   문법 정합성만 확인해라.
...
4. **`wiki/records/handoff/14_배포_계약.md`**(있으면, 없으면 만들지 말고
   `wiki/records/handoff/13`에만 적어라) — 위 제약들을 배포 체크리스트로
...
- 두 프로세스(`customer-runtime`/`composer-control`) 분리를 시작하지
  않는다 — 그건 다음 스트림이다.
- `wiki/records/handoff/13`의 계약 자체(엔드포인트·scope·audit 이벤트 shape)를
...
`wiki/records/reports/2026-08-18_S-COMPOSER-DEPLOY-DOCS-01_리포트.md` — 고친
