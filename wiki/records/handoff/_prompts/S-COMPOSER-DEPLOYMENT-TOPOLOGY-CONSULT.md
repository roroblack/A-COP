...
같은 프로세스, 같은 포트, 같은 바인딩이다. `wiki/records/handoff/13`(Composer
...
- 이번 세션에 basement SemVer/manifest 배포 체계(`scripts/export_basement.py`,
  `wiki/records/handoff/15`)를 막 만들었는데, 이 API 를 별도 서비스로 쪼개면
...
1. **지금처럼 같은 프로세스/포트에 두는 게 실무적으로 문제인가, 아니면
   ...
   (6명·10주 학생 프로젝트, 근거: `wiki/records/handoff/13`이 반복해서 드는
...
2. **대안 — 별도 프로세스/서비스로 쪼갠다면 어떤 형태가 맞나?**
   ...
   - (b) 완전히 별도 프로세스(사이드카)로 배포하되, **같은 basement
     ...
     복제 아님, 배포만 분리)
   - (c) `wiki/records/handoff/13`이 이미 전제하는 대로 VPN/SSH **네트워크
...
4. **지금 당장 뭘 해야 하나** — 이건 학생 프로젝트 규모의 릴리스
   ...
   프로세스 분리는 나중 과제(`wiki/records/vision/`)로 미뤄도 되는지.
