...
`wiki/records/plans/2026-08-18_Composer_배포_경계_분리_계획.md` §4(2단계)가 정의한
...
- 두 서비스 모두 `--workers 1`을 유지한다(`_WRITE_LOCK`이 프로세스
  로컬이라는 기존 제약 — `wiki/records/handoff/13` 참고. 고객 이미지엔 Composer가
...
3. **`wiki/records/handoff/13_composer_쓰기채널_v2_계약.md`**(정확한 파일명은
   실제로 확인해라 — Composer v2 계약 문서) 또는
   `wiki/records/handoff/14_배포_계약.md`(정확한 이름 확인) 중 배포 관련 문서에
   이 분리를 반영 — "이제 이미지 자체가 물리적으로 분리된다"는 사실과,
   "런타임 reload는 여전히 미정의"라는 한계를 함께 적는다.
4. **`wiki/records/plans/2026-08-18_Composer_배포_경계_분리_계획.md`** §4를
...
`wiki/records/reports/2026-08-19_S-COMPOSER-DEPLOY-STAGE2-LOCAL_리포트.md` —
...
- Docker가 설치돼 있다고 가정하고 "빌드 성공"이라고 쓰지 않는다 — 실행
  못 했으면 못 했다고 쓴다.
- `wiki/records/plans/2026-08-18_Composer_배포_경계_분리_계획.md` 직접 수정 금지
  (Claude가 검수 후 반영한다).
