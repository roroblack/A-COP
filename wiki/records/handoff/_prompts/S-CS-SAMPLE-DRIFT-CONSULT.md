...
- `final_project_sample`엔 이 메서드도, 이 config 클래스도 없다 — sample의
  설계 기록(`wiki/records/reports/2026-08-18_Composer_v2_ResponseReview_검수.md`
...
- `register_prompt_files()`의 키 계산 로직(부모 디렉터리 포함)이
  버그였다가 고쳐진 이력이 cs의 `wiki/records/history/2026-08-17_2350_
  handoff계약_동기화_및_프롬프트감사_발견.md`,
  `wiki/records/reports/2026-08-18_S-PROMPT-AUDIT-WIRING_리포트.md`에 남아있다.
...
- sample은 오늘(2026-08-19) `acop_basement`/`acop_composer` 두 pip
  패키지로 쪼개졌다(`final_project_sample/wiki/records/handoff/15_basement_버전_
  배포_계약.md`) — 목표는 "basement가 **범용·재사용 가능한 core**가
  되어, cs 같은 릴리스 대상이 그걸 그대로 pip install 해서 쓴다"는
  것이다(`final_project_sample/wiki/records/handoff/10_도메인_교체_가이드.md`).
...
1. **§1(프롬프트 감사추적 배선)과 §2(Controller 자동배선)는 성격상
   ...
   `final_project_sample/wiki/records/handoff/10_도메인_교체_가이드.md` §0을
...
3. **선택지 비교** — 다음 중 무엇을 권고하는가, 또는 다른 안이 있는가.
   ...
   반영해라)를 같이 적어라.
   - (a) 지금은 그대로 둔다 — 문서(`wiki/records/handoff/10` 또는 새 문서)에
   ...
   - (b) sample의 `acop_basement`에 이 두 메커니즘을 **새로
     ...
     기능 추가 — `wiki/records/handoff/15` SemVer 기준), 이후 cs가 원할 때
...
4. **지금 당장 뭘 해야 하나** — 사용자 지시는 "이건 안 건드림"이다.
   ...
   기록해둬야 하는지 구체적으로 제안해라(예: `wiki/records/vision/`에 새 항목,
   `wiki/records/handoff/10`에 한 절 추가 등 — 이 저장소가 실제로 쓰는 관행에
   맞춰서).
