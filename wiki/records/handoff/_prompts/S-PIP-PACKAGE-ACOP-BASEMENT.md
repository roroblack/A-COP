...
- `wiki/records/handoff/10` §0 경계표, `tests/architecture/test_basement_is_domain_free.py`
...
- 이 저장소가 지금 다른 세션에 의해서도 동시에 편집되고 있을 수 있다
  ...
  `wiki/records/handoff/08`·`09`, `wiki/records/release_checklist.md`, `tests/e2e/test_audience_boundary.py`
...
3. `app/presentation/api/app.py`의 import 3줄만 새 경로로 고친다
   ...
   **`create_app()`의 동작(라우터 등록 여부·순서)은 그대로 유지한다** —
   ...
   `wiki/records/plans/2026-08-18_Composer_배포_경계_분리_계획.md` §4 참고).
...
- 동시에 편집 중인 다른 세션의 파일(`CLAUDE.md`,
  ...
  `wiki/records/handoff/08`·`09`, `wiki/records/release_checklist.md`,
...
`wiki/records/reports/2026-08-18_S-PIP-PACKAGE-ACOP-BASEMENT_리포트.md` — 각
