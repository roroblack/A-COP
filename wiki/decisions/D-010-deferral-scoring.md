...
★**전제 자체가 반증됐다.** 이 문서는 "채점자는 답변 본문이 없으니 `correctness` 를
낮게 준다"에서 출발한다. `[실측 2026-09-07]` **정반대다** — `answer` 가 `null` 인
승인 대기 행 중 통과한 23건의 축 평균이 `correctness 4.00` 이다. 채점자는 다음
행동이 맞고 인용이 유효하면 본문이 없어도 만점을 준다.

`[미확보]` 그게 루브릭 의도인지 채점자의 해석인지는 안 가렸다. `judge_v3.txt` 의
`correctness` 설명에 **답이 없을 때의 규칙이 없다.** 정하지 않은 자리를 채점자가
메우고 있다 — **이 결정보다 그 구멍을 먼저 막아야 한다.**
...
## ~~이 결정 전에는 A/B 수치를 쓰지 않는다~~ → 재측정은 끝났다
...
| **채점식이 승인 대기를 벌점 처리** | **[2026-09-07] 진단이 틀렸다** — 위 실측 두 판(v1·v3) 다 승인 대기가 `respond` 보다 잘 통과한다 |

**첫 번째 수정은 점수를 낮추는 방향이다.** 그래도 고쳤다 — 의도한 판정이 한 번도 걸리지 않은 상태에서 나온 값은 근거가 될 수 없다.

★**[2026-09-07 정정] 이 절의 제목은 이제 맞지 않는다.** 재측정은 2026-09-06 에
끝났고(세 군 216건씩) 수치는 [../evaluation/index.md](../evaluation/index.md) 에 실렸다.
막던 조건 둘 중 하나는 고쳐졌고 **하나는 고쳐진 게 아니라 막을 이유가 없던 것**이다.
이 구분을 지운 채 "둘 다 정리됐다"고 적으면 다음 사람이 승인 대기 벌점이 고쳐진
줄 안다. `rubric.json` 의 `deferral_rule` 은 지금도 `enabled: false` 이고
**이 결정 자체는 여전히 미결이다.**

★**그래서 이 결정이 지금 막는 것은 A/B 수치가 아니라 헤드라인 표기다** —
"정답률 하나로 낼 것인가, 정답률과 안전 기권률 둘로 낼 것인가".
