...
**★ 완료 (2026-09-06, 코드 세션 `f2319aa`).** cs의 복사본 400줄을 지우고 `app/composer_host.py`(스키마·등록표·저장소·인증·경로 어댑터)로 `acop_composer` 패키지에 이 제품의 것을 넘긴다. 관리용 빌드 진입점 `app/entrypoint.py`, 파일 저장소 3종 `app/core/composer_stores.py`, `composer:admin` scope. 사본이 실제로 망가뜨리고 있던 것 — cs에는 `/catalog`·`/changes`가 없어 **콘솔 카드가 cs 대상에서 404**였고, `create_app()`이 쓰기 채널을 고객 릴리즈에 무조건 붙이고 있었다. 아래는 그 전 기록이다. 패키지 주입형 전환 + cs 복사본 제거는 **코드 담당 세션 몫**으로 넘어갔다. 이 문서 세션은 코드를 더 건드리지 않는다. 담당 세션이 볼 것 — 위 표의 셋과 고치는 방법, 그리고 sample에 이미 들어간 구현(커밋 `d05aced`: `composer:admin`·`revision_store.py`·마이그레이션 009·`/revisions`·`/restore`, 전체 494 통과). 끝나면 이 절과 [open-items](../delivery/open-items.md)의 cs 복사본 행을 닫는다.
