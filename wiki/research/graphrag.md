...
| 주장 | 출처 성격 |
|---|---|
| GraphRAG는 Vector RAG 대비 3~5배 비용 | 외부 분석 — `[미확보 2026-09-10]` 출처·조사 시점·분모가 없다. 원문을 다시 찾기 전까지 인용하지 않는다 |
| 엔티티·관계 환각 위험 | 외부 분석 |
| 엔터프라이즈 RAG 구현의 72~80%가 프로덕션 미도달 | 외부 분석 — `[미확보 2026-09-10]` 같다. 비율의 분모(무엇의 72~80%인가)가 없다 |
