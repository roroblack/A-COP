...
`[정정 2026-09-10]` 「유일한 표」는 넓다 — 아래 B·D·E 도 **같은 조건 안에서는** 서로 견준다. 이 표가 특별한 것은 여러 출처를 같은 벤치마크·지표·방향으로 한 줄에 세운다는 점이다.
...
`[외부 2026-09-10]` **방향은 영어→한국어다** — 모델카드가 이 표를 「English to Korean translation」 결과로 적는다(2026-09-10 모델카드를 읽어 확인). 이 절에 방향이 빠져 있었다.
