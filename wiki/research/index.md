...
`[실측]` 사용자 결정 2026-09-09 — **고객 언어로의 번역은 고객 에이전트 몫이다.** 우리는 영어 하나만 낸다. → [D-016](../decisions/D-016-scope-narrowing-reasons.md)

| | |
|---|---|
| **번역 품질 비교** | **우리 지표가 아니다.** 우리 층에서 번역이 일어나지 않는다 |
| **영어 통지 문구 작성 품질** | **우리 몫이다.** 번역이 아니라 작성이다 (Composer 소재, v10 §5-A) |
...
| **인바운드 여행자 행태** | 페르소나와 페인포인트의 실측 근거 | `[미확보]` v10 §1 — 사업 검토서의 근거는 **한국인 해외여행 고객**의 것이라 유추로만 쓴다 |
