...
**E4B Q4_0 은 위 표처럼 실제로 돌았다**(Adreno 825, 8.98 t/s `[실측]`). `[정정 2026-09-10]` 이유를 「4.79 GiB 라 4,592 MiB 에 겨우 들어간다」로 적었는데 **산술이 틀렸다** — `4.79 GiB = 4,905 MiB` 로 여유 메모리보다 크다. **파일 크기와 GPU 에 실제로 올라간 양은 다르다**(mmap·부분 오프로드 등). `[미확보]` 실제 할당량을 재지 않았다. E2B 는 여유가 있다.
