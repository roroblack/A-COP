...
`[실측]` (`[정정 2026-09-10]` 「S2」는 이 문서에 정의되지 않은 출처 번호였다 — 떼어 오기 전 원본의 번호다)
...
**계산상 된다. 하지만 측정이 아니다.** 아래 「x600 실측」 절 참조(`[정정 2026-09-10]` 「§6」으로 적혀 있었다 — 이 문서에는 §6 이 없다. 원본 번호가 남은 것이다).
...
**원본(infrastructure-cost.md) §3 의 "12GB 에서 학습 가능 1,024 토큰" 도 이 절반 위에서 나온 값일 수 있다.**
...
`[추정]` **대화형 세션(로컬 또는 RDP)에서 돌렸다는 뜻으로 읽힌다.** `[정정 2026-09-10]` 「뜻이다」로 확정했었다 — 376스텝 완료만으로 세션 종류를 확정할 수는 없다. 그때 세션 번호를 기록한 것은 이 문서에 없다.
...
`[실측]` 가용량이 **71 MiB 일 때도 있고 5,831 MiB 일 때도** 있었다. `[추정]` 떠 있는 다른 프로그램 때문으로 보지만, 두 관측값만으로 원인을 확정하지 못했다(`[정정 2026-09-10]` 원인을 실측처럼 적었었다).
