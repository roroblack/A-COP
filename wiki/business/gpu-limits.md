...
| 세션 | 상한 |
|---|---|
| 세션 0 (SSH) | **2.25GB** — 측정만 안 될 뿐, GPU 자체 문제가 아니다 |
| **세션 1 (데스크톱)** | **11.75 GiB — 사실상 전량** |

## ★ [2026-09-04] 세션 1 에서 재봤다 — 12GB 가 맞다

`[실측]` **예약 작업으로 세션 1(로그인된 데스크톱)에 파이썬 스크립트를 실행시켰다.** 화면에 콘솔 창이 잠깐 떴다.

```
torch 2.11.0+cu128  cuda=True
NVIDIA GeForce RTX 4070 SUPER  total 11.99 GiB
before: used 71 MiB  free 11929 MiB
  … 1 GiB 씩 늘려가며 …
  11.0 GiB allocated  smi used=11496 free=504
final allocated=11.75 GiB  smi used=11816 free=184
```

**11.75 GiB 를 실제로 잡았다.** 카드가 12GB 면 12GB 를 다 쓴다.

### 그래서 앞선 "6.4GB 제약"은 틀렸다

`[실측]` **재측정 시점의 베이스라인은 71 MiB 다.** 2026-09-03 에 5,831 MiB 로 쟀던 건 **상시 점유가 아니라 그 순간 다른 무언가가 떠 있었던 일시적 값**이었다.

```
2026-09-03 SSH 로 잰 것    used 5,831 MiB   ← 그 순간의 스냅샷
2026-09-04 세션 1 재측정   used    71 MiB   ← 지금의 베이스라인
```

**"Windows 데스크톱이 상시 47%를 먹는다"는 결론이 틀렸다.** 데스크톱 자체는 거의 안 먹고, 그때 떠 있던 다른 프로세스(브라우저·게임 등)가 먹었을 뿐이다.

### 남은 불확실성 하나

`[실측]` **떠 있는 다른 프로그램에 따라 가용량이 달라진다.** 71 MiB 일 때도 있고 5,831 MiB 일 때도 있었다.

`[미확보]` **실제 추론·학습을 돌릴 때 다른 무엇이 떠 있는지는 그때그때 확인해야 한다.** 상시 6.4GB 로 계획하면 안 되고, 상시 12GB 로 계획해도 안 된다 — **돌리기 직전에 `nvidia-smi` 로 확인한다.**

### 측정 방법

**세션 0(SSH)으로는 GPU 작업을 못 잰다.** 예약 작업(`schtasks /it /ru <user>`)으로 세션 1 에 넘기거나, **사람이 직접 로그인**해서 돌린다.

→ [../evaluation/model-selection.md](../evaluation/model-selection.md) 의 메모리 기준이 **12GB** 로 바뀐다.
