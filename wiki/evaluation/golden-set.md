...
domain: commerce
domain_note: 코드가 아직 커머스다 — 여행 전환 층 10(평가 데이터셋 golden.jsonl 이 쇼핑몰) 미완. 문서는 코드를 정확히 적고 있다. 코드가 옮겨지면 이 문서도 같이 옮긴다 — program/plan/A-COP_여행전환_현황_2026-09-09.md
---
...
`[미확보]` **다만 후보가 아예 없는 건 아니다.** [소스 카탈로그](../../datasets/wiki/source-selection.md)에 받지 않은 채 적혀 있는 것 둘 — **KOTE**(온라인 댓글 5만 개, 43감정 라벨, HuggingFace `searle-j/kote`)와 HuggingFace 감정분류 모델들(`Jinuuuu/KoELECTRA_fine_tunning_emotion`의 6종엔 **불안·당황**이 있다). 제품 만족도 축인 `aihub_71603`보다는 이쪽이 가깝다. **그래도 상담 중 상태를 잰 건 아니다** — 댓글 감정이지 문의 감정이 아니고, 모델로 골든셋을 라벨링하면 사람 라벨이 아니라 모델 라벨을 비교하는 게 된다. 축이 맞는지부터 사람이 판단해야 한다.
...
4. **intent 비율이 실제 문의 분포와 같은지 모른다.** 균등 분할이지 비례 표집이 아니다 (위). `[정정 2026-09-10]` 「다르다」로 적혀 있었는데 위 절이 적은 대로 **실제 분포는 모른다** — 다르다고도 같다고도 말할 수 없다
