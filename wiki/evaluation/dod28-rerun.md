...
`[정정 2026-09-10]` **이 제목은 틀렸다** — 재측정은 있었다(v8·v9, 아래 「그 뒤 실험이 더 있었다」 절). 이 절은 그 실험을 못 본 채 쓴 서술이다.
...
| # | 할 일 | 막는 것 |
|---|---|---|
| 1 | **D-010 을 정한다** | 채점식이 안 정해지면 뭘 재도 다시 재야 한다 — `[2026-09-10]` 위에서 재측정을 「차단 항목에서 뺀다」고 했으니 이 순서는 차단이 아니라 **권하는 순서**로 읽는다. 지금 기권 규칙은 꺼져 있다(`final_project_cs/eval/judge/rubric.json` `deferral_rule.enabled: false`) |
