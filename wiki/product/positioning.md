...
## ★ [2026-09-01 추가] 경쟁 대비 격차

`[실측]` `_시장·출처근거.md`에서 이관. **이 절이 통째로 빠져 있었다.** 위의 "자체호스팅은 차별점"이 왜 차별점인지가 wiki 어디에도 없었다.

| # | 격차 | 등급 |
|---|---|---|
| **G1** | **Intercom Fin 은 단일 에이전트다.** multi-agent·agent-to-agent·전문 라우팅 근거가 공개되지 않았다 | 미공개 |
| **G2** | **Fin 에 자체 호스팅·온프레미스·air-gapped 선택지가 없다.** 규제 산업에서 장애가 된다 | 공개 자료 |
| **G3~G7** | 업계는 **데모 자동화율과 실제 운영 성과 사이에 차이가 있다.** 품질관리·감사·승인·프로세스 오케스트레이션이 차별화 지점 | 2차 |
| **G7** | 컴플레인·정책 예외·서비스 회복은 **사람 판단으로 넘길 경계**가 필요하다 | — |
| **E3~E5** | **해결당 과금**과 통합 범위도 구매 대안의 근거다 | 2차 |
| **H1~H5** | UCP·AP2·A2A·MCP 가 에이전트 커머스·결제 위임·상호운용성의 관련 표준 | 1차 |

**G2 가 위 "자체호스팅의 논거"를 받친다.** 원가로는 못 이기지만 **선택지 자체가 없는 것**과 있는 것의 차이다.

**G7 이 [D-005](../decisions/D-005-write-gate.md) 를 받친다.** 사람 판단 경계는 우리만의 발상이 아니라 업계가 이미 요구하는 것이다.

### 이 근거를 쓸 때 지킬 것

`[실측]` 원문의 주의사항 넷을 그대로 옮긴다.

| 규칙 | 왜 |
|---|---|
| **Fin 에 대한 근거를 Sierra·Decagon 전체로 확장하지 않는다** | 확인한 것은 Fin 하나다 |
| **자체 호스팅·품질관리·주문/반품 검증은 계획과 구현 상태를 분리한다** | A-COP 의 자체 배포는 **제품 방향이지 구현 완료가 아니다** |
| **시장 수치는 1차/2차/추정 등급을 명시한다** | B·C·D·E·F·G 의 다수가 2차 인용이다 |
| **UCP/AP2 세부 수치는 원문 확인 전 발표용 확정값으로 쓰지 않는다** | |

**둘째 줄이 가장 위험하다.** "자체호스팅 된다"고 말하는 순간 구현 완료를 주장하는 게 된다.
