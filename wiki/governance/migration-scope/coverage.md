...
## 6차 이후 — 2026-09-04~06 대조는 별도 문서에

`[실측]` DoD evidence 28건 · evidence 검증 로그 5건 · 데이터셋 5건 · cs 계약·매뉴얼 4건 + WBS 원본 · acop_dojo 2건 · VISION 9건 — **53건**을 직접 대조했다. 이 파일이 300줄을 넘어 떼어 냈다.

→ [coverage-2026-09.md](coverage-2026-09.md)

**거기서 나온 것** — wiki 자체가 틀려 있던 곳이 여럿(dod.md·shared-state·a2a-protocol·outbox·rest-api·acop_dojo index 등, 목록은 각 차수 표), 새 페이지 2(`graph-retrieval`·`dod-evidence-drift`), evidence 원본이 낡은 9건은 cs 저장소 작업자 몫. **적중률은 읽을 순서를 정해 줄 뿐 판정을 대신하지 못한다** — 스캔이 맞은 것과 틀린 것이 둘씩 있었다.
...
| 4 | 남은 `유지` 문서 33건 — 스캔 순위대로 진행 중. `판정필요` 68건은 사람 몫 |
