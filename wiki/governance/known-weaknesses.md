...
| `index` 와 `log` 이중 관리 | PR 마다 누락 | **일어났다.** `index` 연결을 몇 번 놓쳤다 |
...
| **`CLAUDE.md` 가 다시 지식 창고가 됨** | 라우터가 아니라 내용을 담는다 | **절반 일어났다** `[실측 2026-09-10]` — **가리키는 것은 됐다**(루트 `CLAUDE.md` 가 「읽는 순서는 wiki 먼저」라고 적고 wiki 를 10회 언급한다). 그런데 **「현재 기준 사실」 표 20여 행을 직접 담는다** — 라우터이면서 지식도 담는다. `[정정]` 「아직 wiki 를 안 가리킨다」로 적혀 있었다 |

**아홉 중 다섯이 실제로 일어났다**(위 표에서 「일어났다」 5행 · 막았다 1 · 안 일어났다 1 · 아직 1 · 절반 1). `[정정 2026-09-10]` 「여섯」으로 적혀 있었다 — 세면 다섯이다. 예측이 쓸모 있었다.

### ★ [2026-09-03] 예측에 없던 실패 하나

**폴더 승격이 링크를 조용히 깬다.**

`[실측]` 파일을 폴더로 올릴 때 **그 폴더 안 문서들의 상대경로가 한 단계 깊어진다.** 밖에서 들어오는 링크는 고쳤는데 **안에서 나가는 링크를 놓쳤다.**

```
teams/team-contract.md          →  teams/team-contract/index.md
  안에 있던 [team-boundary.md](team-boundary.md) 가
  teams/team-contract/team-boundary.md 를 찾는다
```

**네 곳에서 났다** — `team-contract/`, `migration-scope/`, `type-verification/`, `data/schema/`.

`[실측]` **검사기가 잡았다.** "미작성 문서로 가는 링크"로 보고했고, 실재하는 파일인데 그렇게 나와서 이상해 찾았다.

**승격할 때 안팎을 다 봐야 한다.** → [structure-guide.md](structure-guide.md)
