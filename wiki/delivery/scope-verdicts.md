...
|---|---|---|
| **분모를 오염시키지 않는다** | 수리 사례를 청약철회 평가 분모에서 뺐다 | 여행 골든셋을 만들 때 **성격이 다른 케이스를 한 분모에 넣지 않는다.** `[정정 2026-09-10]` 예시로 「예약 있는 액티비티와 무료 관람은 판정 규칙이 다르다」고 적었는데 **v11 §5-D 와 충돌한다** — **새 업무 규칙을 만들지 않는다.** 둘은 **같은 규칙**을 쓰고 **판정 입력이 다르다**(예약 대 일정 항목). 그래서 골든셋에는 **둘 다 넣고 입력 종류로 층을 나눠 센다** — 한쪽이 전체 점수를 가리지 않게 |
