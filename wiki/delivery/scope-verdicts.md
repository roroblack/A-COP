...
**아래 안건 일곱은 전부 쇼핑몰 데이터다** — 판매자취소·품질보증·쿠폰·송장. v10 §0-2 가 커머스 도메인을 뺐으므로 **판정 자체는 더 이상 적용되지 않는다.**
...
| **Mock 을 정답으로 쓰지 않는다** | contract·E2E fixture 로만 썼다 | **이 프로젝트에서 훨씬 커졌다.** 협약사 자동 실행이 Mock 공급자 한정 시연이다 — Mock 결과를 실적으로 읽으면 v10 §10 이 세운 리스크가 그대로 실현된다 |
| **비결정적인 것을 합격 경로에 두지 않는다** | 실송장 조회를 보조 시연으로 뺐다 | **외부 API(기상·관광정보·경로)가 전부 비결정적이다.** 기록 재생을 합격 경로로 두고 실호출은 보조 시연으로 둔다 |
| **제외의 범위를 좁게 말한다** | "실물 배송 범위에서의 제외이지 영구 제외가 아니다" | v10 §9-A 가 뺀 열두 개도 같다 — **범위를 자른 것이지 불가능하다는 결론이 아니다** |
...
★**여행에서 더 걸린다.** 여행 일정은 **언제 어디에 있었는지의 기록**이라 그 자체가 위치 이력이다. v10 §11 이 예약·현장 데이터를 **합성**으로 두는 이유이기도 하다.
