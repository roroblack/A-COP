...
(() => {
  ...
  const PHONE_PATTERN = /01[016-9](?:[-\s]?\d{3,4}[-\s]?\d{4}|\*{4}\d{4})/g;
  ...
  function initialState(config = {}) { return { phase: 'INIT', scope: config.collectionScope || config.scope || 'tracking', years: [], yearIndex: 0, page: 1, orders: [], tracking: [], queue: [], cursor: 0, warnings: [], done: false, yearScope: config.yearScope || 'all', pageCount: 0, listSignature: null, listUrl: null, listKey: null, restore: null, seenKeys: [], stalls: 0, forceSkip: false }; }
