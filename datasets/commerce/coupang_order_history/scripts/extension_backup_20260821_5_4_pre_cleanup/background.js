...
(() => {
  ...
  function createController(chromeApi, options = {}) {
    ...
    async function cancellableSleep(ms, signal) {
      ...
      try { return await Promise.race([Promise.resolve(sleep(ms)).then(() => true), cancelled]); }
    ...
    async function rateLimit(config, job, signal) {
      ...
      if (!await cancellableSleep(waitMs, signal)) return false;
      if (job) {
    ...
    // 워커가 종료되더라도 tabs.onUpdated와 alarm이 저장된 체크포인트에서 다시 깨운다.
    function scheduleResume(delayMs = pollMs) {
    ...
    async function recoverTab(job) {
      ...
      // Chrome이 실제로 discarded/unloaded라고 보고한 탭만 깨운다.
    ...
    async function loop(maxIterations = 20, signal, autoSchedule = false) {
      let iterations = 0;
      while (iterations < maxIterations) {
        if (signal?.aborted) return getJob();
        iterations += 1;
        ...
        // 실제 저장 작업(startedAt 보유)만 한 번 초기화하고, 수동 테스트용 상태는 건드리지 않는다.
        if (job.startedAt && job.engineVersion !== ENGINE_VERSION) {
          const legacyHelperTabId = job.helperTabId;
          job.engineVersion = ENGINE_VERSION;
          job.state = freshState(job.config);
          job.progress = { stage: 'INIT', page: 1, count: 0, remaining: 0, message: '새 문서 탐색 방식으로 수집을 다시 시작합니다.' };
          job.result = null;
          delete job.helperTabId;
          delete job.pendingNavigation;
          if (!await saveIfRunning(job)) return getJob();
          if (legacyHelperTabId && legacyHelperTabId !== job.tabId && chromeApi.tabs.remove) {
            try { await chromeApi.tabs.remove(legacyHelperTabId); } catch { /* 이미 닫힌 이전 보조 탭 */ }
          }
        }
        ...
        try {
          ...
          if (!await cancellableSleep(pollMs, signal)) return getJob();
          continue;
        }
        if (signal?.aborted) return getJob();
        ...
        if (step.action.type !== 'none') {
          ...
          if (!await saveIfRunning(job)) return getJob();
          if (!await rateLimit(job.config, job, signal)) return getJob();
      ...
      const job = await getJob();
      if (autoSchedule && job?.status === 'running' && !signal?.aborted) scheduleResume(pollMs);
      return job;
    }
    ...
    function beginResume(maxIterations, autoSchedule) {
      if (loopPromise) { resumeRequested = { maxIterations, autoSchedule }; return loopPromise; }
      const signal = generationController.signal;
      loopPromise = loop(maxIterations, signal, autoSchedule).finally(() => {
        loopPromise = null;
        if (resumeRequested) {
          const requested = resumeRequested;
          resumeRequested = null;
          void beginResume(requested.maxIterations, requested.autoSchedule);
        }
      });
      return loopPromise;
    }

    function resume(maxIterations) {
      const autoSchedule = arguments.length === 0;
      return beginResume(autoSchedule ? 20 : maxIterations, autoSchedule);
    }
    ...
    async function start(tabId, config = {}, startOptions = {}) {
      ...
      const job = {
        ...
        engineVersion: ENGINE_VERSION, status: 'running', tabId, config,
  ...
  if (!globalThis.__coupangPopup && typeof chrome !== 'undefined' && chrome.runtime?.onMessage) {
    ...
    chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
      if (message?.type === 'START') {
        ...
        // 구동자는 서비스 워커 하나뿐이다. 팝업이 닫혀도 저장된 상태에서 계속 간다.
      ...
      if (message?.type === 'GET_JOB') {
        // 상태 조회가 잠든 워커를 깨운 경우 진행 중인 작업도 이어간다.
        controller.getJob()
    ...
    try {
      chrome.tabs.onUpdated.addListener((tabId, info) => {
        ...
        controller.getJob().then((job) => {
          if (job?.status === 'running' && job.tabId === tabId) void controller.resume();
        }).catch(() => {});
      });
    } catch (error) {
    ...
    try {
      ...
      startupError = `탭 종료 이벤트 등록 실패: ${error.message}`;
    }
    try {
    ...
    const resumeAfterBrowserStart = () => {
      controller.getJob().then((job) => {
        if (job?.status !== 'running') return;
        chrome.alarms.create(ALARM_NAME, { periodInMinutes: 0.5, when: Date.now() + FIRST_ALARM_DELAY_MS });
        void controller.resume();
      }).catch(() => {});
    };
    try { chrome.runtime.onStartup?.addListener(resumeAfterBrowserStart); }
    catch (error) { startupError = `브라우저 시작 이벤트 등록 실패: ${error.message}`; }
    try { chrome.runtime.onInstalled?.addListener(resumeAfterBrowserStart); }
    catch (error) { startupError = `확장 갱신 이벤트 등록 실패: ${error.message}`; }
    // 모듈 시작 자체는 루프를 만들지 않는다. START, 탭 로드, 알람, GET_JOB 이벤트가 깨운다.
