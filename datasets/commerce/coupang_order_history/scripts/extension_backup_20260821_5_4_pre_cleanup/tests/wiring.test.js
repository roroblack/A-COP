...
function loadWorker({ initialJob = null, onRunStep, setDelays = [], getError = null } = {}) {
  ...
  const fireAlarm = (name = 'coupangCollectorKeepAlive') => {
    for (const listener of listeners.alarm) listener({ name });
  };
  const finishTabLoad = (tabId = 7) => {
    for (const listener of listeners.tabUpdated) listener(tabId, { status: 'complete' });
  };
  return { data, send, calls, fireAlarm, finishTabLoad };
}
...
test('상세 페이지 로드가 끝나면 저장된 작업을 이어간다', async () => {
  const job = {
    status: 'running', tabId: 7, config: {},
    state: { phase: 'DETAIL', orders: [], warnings: [] }, progress: {}, result: null
  };
  const { data, calls, finishTabLoad } = loadWorker({ initialJob: job, onRunStep: (state) => DONE_STEP(state) });

  finishTabLoad(7);
  await new Promise((resolve) => setTimeout(resolve, 60));

  assert.ok(calls.runStep >= 1, '상세 페이지가 로드됐는데 수집을 재개하지 않았다');
  assert.equal(data.coupangJob.status, 'completed');
});

test('알람이 잠든 서비스 워커의 저장된 작업을 이어간다', async () => {
  const job = {
    status: 'running', tabId: 7, config: {},
    state: { phase: 'DETAIL', orders: [], warnings: [] }, progress: {}, result: null
  };
  const { data, calls, fireAlarm } = loadWorker({ initialJob: job, onRunStep: (state) => DONE_STEP(state) });

  fireAlarm();
  await new Promise((resolve) => setTimeout(resolve, 60));

  assert.ok(calls.runStep >= 1, '알람이 왔는데 수집을 재개하지 않았다');
  assert.equal(data.coupangJob.status, 'completed');
});
