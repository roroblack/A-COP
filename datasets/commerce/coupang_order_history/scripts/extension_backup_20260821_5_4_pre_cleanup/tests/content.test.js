...
test('주문 JSON과 배송 JSON을 분리한다', () => {
  ...
  assert.deepEqual(exports.trackingData, [{
    courier: '로켓배송', tracking_number: '10327825750572', shipment_box_id: null, order_id: '16102412730885', status: '배송완료',
    events: [{ kind: '배송완료', where: '안양1', timeString: '2026-08-20T14:07:00.000Z', raw: '어제 23:07' }],
    queried_at: '2026-08-21T01:00:00.000Z'
  }]);
});

test('송장번호가 비어도 배송 이력을 내보내고 배송박스별로 구분한다', () => {
  const event = { kind: '상품준비중', where: '물류센터', timeString: '2026-08-21T01:00:00.000Z' };
  const exports = buildExportPayloads([
    { OrderId: 'same-order', _shipmentBoxId: 'box-a', TrackingNumber: null, TrackingEvents: [event], TrackingEventRaw: ['원문 A'] },
    { OrderId: 'same-order', _shipmentBoxId: 'box-b', TrackingNumber: null, TrackingEvents: [event], TrackingEventRaw: ['원문 B'] }
  ], { exportedAt: '2026-08-21T02:00:00.000Z' });

  assert.equal(exports.trackingData.length, 2);
  assert.deepEqual(exports.trackingData.map((entry) => entry.shipment_box_id), ['box-a', 'box-b']);
  assert.ok(exports.trackingData.every((entry) => entry.tracking_number === null));
});
