...
${ORDER_LIST_URL}`); return tab; }
  ...
  function renderStatusLines(job) {
    ...
    const position = progress.year ? `${progress.year}년 ${progress.page || 1}페이지` : `${progress.page || 1}페이지`;
    const lines = [
  ...
  // 수집 루프는 서비스 워커 하나만 돌린다. 팝업은 상태 표시와 명령만 담당한다.
  ...
  elements.stateProbe.addEventListener('click', async () => {
    ...
    try {
      ...
      if (health) {
        ...
        lines.push(`워커 ${health.looping ? '실행 중' : '이벤트 대기'} · 체크포인트 재개 ${health.eventDriven ? '켜짐' : '꺼짐'} · 다음걸음 예약 ${health.wakeScheduled ? '있음' : '없음'} · 마지막 걸음 ${since === null ? '없음' : `${since}초 전`}`);
