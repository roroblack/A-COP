...
`[실측 2026-09-10]` `courier_tracking/processed/tracking.jsonl`은 **236건**이다 `[정정]` 「여전히 첫 수집분 57건」으로 적혀 있었다. 그 전 서술 — 첫 수집분 57건이다(원본이 날짜별 두 파일 58줄인데 **같은 송장 1건이 08-21 재조회로 양쪽에 있어** 최신 조회분만 남겼다 — `_일일작업_2026-08-28.md` §6. 58과 57 중 어느 쪽이 맞는지 적어 두지 않으면 세는 사람마다 다른 답을 낸다). 제출본 5명분은 `raw/_incoming_20260829/`에 있고 `processed/`로는 안 합쳤다 — 합본은 `_dist/`의 파생본이다.
...
`[실측]` 2026-09-06 디스크 확인. [REPORT.md](../voc/data_go_kr_consumer_complaints/REPORT.md)와 어긋나는 게 둘이다.
