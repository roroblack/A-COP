...
| 폴더 | 역할 |
|---|---|
| [`final_project_cs/`](final_project_cs/) | 릴리스 대상 제품 코드베이스다. 실제 CS Pack·Commerce Ops Team을 구현하며, 현재 상태표 기준으로 Core 런타임, REST/MCP, Billing/Technical Agent Team, 쇼핑몰 도메인 RAG·Feedback Analytics·Controller·운영 UI·A2A/Graph·ActionProposal 방어·프롬프트 감사추적이 구현·검증돼 있다. DoD는 evidence 28/28, 통과 24, 부분통과 4이며 29번(Response Generation & Review)은 아직 평가되지 않았다. RC는 아니다. |
