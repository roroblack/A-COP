...
`[실측]` v10 §5(2026-09-09 갱신) · `A-COP_여행Team모듈_구성안.md`.

| 예시 | 기상 감시가 의미 있나 | 예약 시각이 고정인가 |
|---|---|---|
| **골프 (티타임)** | **예** — 우천 위약금 규정이 문서로 존재한다 | **예** |
| 한강 수상 액티비티 · 겨울 스키·눈썰매 | 예 | 예 |
| 공연·뮤지컬 · 쿠킹 클래스 | **아니오** (실내) | 예 |
| 테마파크 | 약함 | **아니오** (날짜권) |
| 템플스테이 | 약함 | 숙박에 가깝다 — Lodging 과 겹친다 |

★**골프가 이 Team 규칙에 가장 잘 맞는다.** 우천 위약금 규정이 문서로 존재하고 티타임이 고정이다 — 판정 규칙(날씨 조건)과 감시 소스(기상청 초단기예보) 둘 다 그대로 걸린다. **초판 예시 목록에 빠져 있었고 2026-09-09 에 첫 자리로 들어갔다.**

### ★ 미해결 — 정의와 예시가 어긋난다

**이건 예시 고르기 문제가 아니라 Team 경계 문제다.** v10 §5 는 Team 을 **객체 종류별**로 나눈다고 했는데, 실내 티켓류를 Activity 에 넣으면 **감시 소스가 둘로 갈린다**(기상청 vs 공연 취소 공지). 한 Team 이 성격이 다른 판정을 두 벌 갖게 되어 그 분할 원칙과 충돌한다.

| 갈래 | 무엇 |
|---|---|
| ① 정의를 넓힌다 | 감시 소스를 둘로 둔다. 분할 원칙과 계속 부딪힌다 |
| ② **레저로 좁힌다** | 골프·수상·스키만 Activity. 티켓류는 별도 Team 또는 등록만 |
| ③ 예시만 고친다 | **지금 상태.** 세부 설계에서 다시 걸린다 |

`[미확보]` 인바운드에서 골프·스키가 티켓류보다 큰지 안 봤다. **①·② 판단에는 그 수요 비교가 필요하다.** 이 문서를 쓰는 시점에 정의는 ③이므로, 아래 판정 규칙은 **레저 상품 기준**으로 읽는다.
