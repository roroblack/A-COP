...
- `wiki/records/reports/2026-08-20_S-GOLDEN-EXPECTED-CAPABILITY_리포트.md` (직전
  작업) 가 `eval/datasets/golden.jsonl` 의 order/shipping/return/exchange
  60건에 `expected_capability` 를 채웠다. 원 계약서
  (`wiki/records/handoff/_prompts/S-GOLDEN-EXPECTED-CAPABILITY.md`) 는 "판단이
...
- `eval/verify_expected_capability.py` (수정 시에만, null 허용 로직 추가
  등 — 불필요하면 건드리지 마라)
- `wiki/records/reports/2026-08-24_S-GOLDEN-CAPABILITY-REAUDIT_리포트.md` (신규)
