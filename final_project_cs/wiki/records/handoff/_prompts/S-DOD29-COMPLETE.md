...
- `wiki/records/evidence/DoD-29_ResponseGenerationReview.md` — 현재 판정 "부분 통과".
...
- 계약 모델(`app/core/contracts.py`) 을 변경해야 한다면 `wiki/records/handoff/01_계약_Pydantic.md`
...
- `app/core/contracts.py`, `wiki/records/handoff/01_계약_Pydantic.md` (계약 변경 시,
  변경한다면 둘 다 반드시 같이)
- `eval/datasets/golden.jsonl`, `eval/datasets/holdout.jsonl`
- `tests/` 아래 신규 테스트 파일(정확한 위치는 기존 통합테스트 배치 규칙을
  따라라)
- `wiki/records/reports/2026-08-20_S-DOD29-COMPLETE_리포트.md` (신규 작성)
...
- `wiki/records/evidence/DoD-29_ResponseGenerationReview.md` 자체를 직접 수정
