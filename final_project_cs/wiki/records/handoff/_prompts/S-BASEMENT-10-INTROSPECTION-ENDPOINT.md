...
(`wiki/records/handoff/12_introspection_계약.md` 참고, scope `ops:introspect`)
로 노출했다.

cs는 이미 같은 이유로 Composer를 분리했다 — `/ui/composer`가
2026-08-18 삭제되고 `final_project_ui`의 인증된 `/composer/*` API로
옮겨졌다(`wiki/records/handoff/09_Composer_GUI_계약.md` 참고). **admin/introspection
...
- `python -m pytest -q -m "not live"` 전체 실행 결과를 리포트에 붙여라
  (378 passed 기준 변화 명시).
- 여유가 있으면 `wiki/records/handoff/13_introspection_계약.md` 신규로 API
...
- 관련 테스트 파일들 (신규)
- `wiki/records/handoff/13_introspection_계약.md` (신규, 선택)
- `wiki/records/reports/2026-08-24_S-BASEMENT-10-INTROSPECTION-ENDPOINT_리포트.md` (신규)
