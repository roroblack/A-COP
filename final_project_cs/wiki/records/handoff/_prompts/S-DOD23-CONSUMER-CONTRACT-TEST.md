...
- `wiki/records/evidence/DoD-23_consumer_idempotency.md` — 지금 consumer 는
...
2. 기존 `OutboxWorker` 를 이 계약 테스트에 실제로 통과시켜서, 계약 테스트
   ...
   안 돌려보면 의미가 없다.
3. `wiki/records/handoff/`(정확한 위치는 기존 계약 문서들 배치 규칙을 따라라)에
...
- `wiki/records/handoff/` 아래 신규 문서 1개(파일명은 기존 `0N_주제.md` 넘버링
  관례를 따르되, 다음 번호를 실제로 확인하고 정해라)
- `wiki/records/reports/2026-08-20_S-DOD23-CONSUMER-CONTRACT-TEST_리포트.md` (신규)
...
- `wiki/records/evidence/DoD-23_consumer_idempotency.md` 직접 수정(Claude 가
  검증 후 갱신한다)
