...
- `wiki/records/evidence/DoD-28_파인튜닝_방어지표.md` — 방어지표 5종(grounding_match,
...
2. 그 출력들에 `eval/defense_metrics.py` 의 5개 지표를 실제로 적용해서
   분자/분모와 함께 기록한다. `wiki/records/evidence/DoD-28_파인튜닝_방어지표.md`
...
- `wiki/records/reports/2026-08-20_S-DOD28-DEFENSE-METRICS-GOLDEN-RUN_리포트.md` (신규)
...
- API 키가 없어서 실 LLM 호출을 못 했다면 가짜 숫자를 채우지 않는다 —
  "미실행"이라고 정직하게 적는다
- `wiki/records/evidence/DoD-28_파인튜닝_방어지표.md` 직접 수정(Claude 가 검증
  후 갱신한다)
