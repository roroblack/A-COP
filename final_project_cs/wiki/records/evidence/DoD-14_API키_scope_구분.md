...
- 실행: 2026-08-12 23:20 · 실측 원문 `wiki/records/evidence/_raw/DoD-14.md`
- 판정: 통과
...
- ★`test_case_from_other_customer_is_not_found` — 남의 Case 는 **404** 다.
  403 이면 존재 사실을 알리는 것이라 실패로 판정한다(`wiki/records/handoff/03` §2).

## ★이 항목은 한 번 전면 실패했었다

1차 산출물은 `security.py` 가 `os.getenv` 로 설정을 읽어 **인증이 필요한 모든 요청이 500** 이었다.
그때 테스트는 정적 단언 3건뿐이라 초록이었다.
→ `wiki/records/reports/debugs/2026-08-12_1830_S-API가_실행되지_않는다.md`
...
`wiki/records/reports/2026-08-24_S-BASEMENT-07-COMPOSER-SECRET-FAILOPEN_리포트.md`,
`wiki/records/reports/2026-08-24_S-BASEMENT-08-COMPOSER-IMPL-ALLOWLIST_리포트.md`
...
- `list_cases()` 동률(같은 `created_at`) 정렬 비결정성 —
  `wiki/records/evidence/DoD-03_동시성_appendonly_replay.md`에도 안 맞고(동시성이
  아니라 정렬 안정성 문제), v8 §27 의 1~29 항목 어디에도 명시적으로
  안 걸린다. `wiki/records/reports/2026-08-24_S-BASEMENT-09-LIST-CASES-ORDERING_리포트.md`만
  근거로 남긴다.
- `GET /introspection`(외부 콘솔용 read-only 조립 스냅샷) — v8 계획서에
  아예 없는 항목이다(basement 완성도 문제이지 v8 DoD 요구사항이 아니다).
  `wiki/records/reports/2026-08-24_S-BASEMENT-10-INTROSPECTION-ENDPOINT_리포트.md`만
  근거로 남긴다.
