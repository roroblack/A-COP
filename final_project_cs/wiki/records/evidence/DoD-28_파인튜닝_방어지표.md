...
근거: `wiki/records/reports/2026-08-20_S-DOD28-DEFENSE-METRICS-GOLDEN-RUN_리포트.md`,
...
상세: `wiki/records/reports/2026-08-28_S-DOD28-GOLDEN-DEFENSE-METRICS_리포트.md`,
...
실행계획 `wiki/records/plans/2026-08-16_v7_격차해소_실행계획.md` 의 P4 → P8 순서가 이것이다.
...
**stage3(v1~v7) 채택하지 않는다.** loss·accuracy·유효 JSON 비율이
전부 개선됐지만, 모델이 실제로 배운 건 "evidence를 보고 판단"이 아니라
"대체로 draft를 그대로 돌려주면 맞더라"라는 지름길이었다는 게 실 사례로
증명됐다.

★★2026-08-31 갱신 3 — 처방(의도적 불일치 데이터)을 실제로 적용해봤다.
draft-evidence가 의도적으로 어긋나는 케이스 30건(배송상태 evidence를
다른 진짜 값으로 바꿔치기)을 만들어 기존 172건과 합쳐 202건(v8)으로
재학습 — holdout 33건(불일치 4건 포함) 비교에서 **mismatch 케이스의
echo율이 v7의 100%(n=1)에서 v8의 50%(n=4)로 떨어졌다** — 모델이
더 이상 무조건 echo하지 않는다는 신호. 다만 재작성한 2건 중 1건만
정확했다(나머지는 재작성했지만 여전히 틀림) — "언제 재작성할지"는
어느 정도 배웠지만 "정확히 재작성하는지"는 불안정하다. n=4라
통계적 결론은 못 낸다. **v8도 아직 채택하지 않지만, 가설(§7.5)이
실제 처방으로 이어졌을 때 지표가 움직인다는 것은 확인했다** — 다음은
mismatch 비율(현재 15%)과 표본을 더 늘리는 것.

전체 경위: [`2026-08-30_DoD28-FT-RAG통합_설계.md`](../plans/2026-08-30_DoD28-FT-RAG통합_설계.md) §5~§7.

★★2026-09-02 갱신 4 — **처방의 확대를 검정했고 재현되지 않았다.**
갱신 3 이 남긴 다음 단계(mismatch 비율·표본 확대)를 실행했다. shipment
합성 케이스 30→90건, mismatch 23→63건(전체의 11%→24%), 층화 분할로
holdout mismatch 를 4→**10건**으로 늘려 통계적 의미를 확보했다(v7 은 1건,
v8 은 4건이라 결론을 못 냈다). stage3-v9 는 `train_loss=0.498`·
`token_accuracy=0.902` 로 지표는 v8 보다 좋아졌다.

★**그런데 먼저, v8 까지 쓰던 `grounded` 지표가 mismatch 에서 무효였다.**
judge 가 evidence 대로 올바르게 고친 답을 "draft 와 모순된다"며 0점 처리한
사례를 실측했다(judge 프롬프트는 evidence 가 유일한 진실이라고 적어 뒀는데도).
그 결과 mismatch 에서 local_ft 와 OpenAI 의 grounded 평균이 0.4 로 같다 —
한쪽은 echo 6/10, 다른 쪽은 0/10 인데도 구분되지 않는다. v8 의 결론도 같은
무효 지표 위에 있었다. judge 를 거치지 않고 상태값을 직접 대조하는 방식으로
바꿨다(`eval/finetune/analyze_stage3_review.py --holdout`).

새 지표로 v8 도 다시 계산한 비교:

| | v8 (n=4) | v9 (n=10) |
|---|---|---|
| local_ft 가 evidence 상태값 채택 | 2/4 (50%) | **4/10 (40%)** |
| OpenAI 채택 | 4/4 (100%) | 7/10 (70%) |
| local_ft echo 율 (non-mismatch) | 90% | **100%** |

mismatch 학습 데이터를 2.7배로 늘렸는데 정답률은 움직이지 않았고
(50%→40%, 둘 다 동전 던지기), non-mismatch echo 율은 오히려 100% 로
굳었다. **v8 의 "개선"은 표본 잡음이었다.**

★**결론(확정): 파인튜닝 모델을 채택하지 않으며, 이 처방의 확대도 여기서
멈춘다.** 더 늘려서 될 일이었다면 23→63건에서 방향이라도 보였어야 한다.
DoD-28 은 "파인튜닝 대안을 실측으로 비교하고 방어지표로 판정한다"는 항목이고,
비교·판정은 완료됐다 — 판정 결과가 "채택하지 않음"일 뿐이다.

전체 경위: [`2026-08-30_DoD28-FT-RAG통합_설계.md`](../plans/2026-08-30_DoD28-FT-RAG통합_설계.md) §7.7.

★★2026-09-02 갱신 5 — **요구사항의 "CI 기록"을 뒤늦게 채웠다.**

DoD-28 원문은 "…분모와 **CI** 를 기록"이라고 요구하는데, 분모만 기록해 왔고
**신뢰구간은 한 번도 기록된 적이 없었다.** 이 문서와 지표 JSON 둘 다에
없었다(2026-09-02 실측, Codex 교차검증에서도 같은 지적).

`python -m eval.stats.defense_ci --iterations 10000 --output eval/reports/2026-09-02_golden_defense_ci.json`

| 지표 | 점추정 | 95% CI |
|---|---|---|
| 근거 정합률 | 0.9701 (681/702) | **[0.9609, 0.9797]** |
| 근거 초과율 | 0.0299 (21/702) | **[0.0203, 0.0391]** |
| 적절한 기권율 | 측정 불가(분모 0) | — |
| 과잉 기권율 | 0.3500 (21/60) | **[0.2333, 0.4667]** |
| 스키마 준수율 | 1.0000 (60/60) | **[1.0000, 1.0000]** |

★**케이스를 재표집했다. 필드가 아니다.** 근거 정합률의 분모 702 는 60 케이스에
걸친 필드 수라, 필드를 독립 표본으로 보고 이항 CI 를 씌우면 구간이 실제보다
좁아진다(같은 케이스 안의 필드는 서로 상관이 있다). 그래서 표본 단위를 케이스로
두는 cluster bootstrap 10,000 회를 썼고, 계산은
`eval/defense_metrics.py::score()` 를 그대로 재사용해 점추정치가 기존 기록과
정확히 일치함을 확인했다.

★**과잉 기권율의 구간이 [23%, 47%] 로 넓다.** n=60 이면 이 정도가 정직한
폭이다 — 점추정치 35% 만 적어 두면 이 불확실성이 숨는다.

★그럼에도 **DoD-28 은 여전히 부분 통과다.** CI 는 채웠지만 요구사항의 다른 두
조각이 남는다(2026-09-02 Codex 교차검증으로 확인):

1. **알파 2차가 알파 실데이터가 아니다** — golden judge-pass 22건으로 대체했다.
   실제 고객 응대 로그는 운영 전환 이후에나 생기므로 **지금은 채울 수 없다.**
2. **holdout 에서 방어 지표 5종을 기록하지 않았다** — judge 비교로 대체했다.
   이건 golden 에서 한 것과 같은 방식으로 채울 수 있다(미실행).

★★2026-09-03 갱신 6 — **CI 수치 정정, 그리고 holdout 방어지표가 왜 안 재지는가**

갱신 5 에 적은 CI 는 **`verification_policy.py` 수정 전 수치였다.** 그 사이
다른 작업이 `calculation_basis` 를 `ignored` 에 넣었는데(어디에도 선언돼 있지
않아 **모든 환불 제안이 승인 직전 재검증에서 "선언되지 않은 필드"로 막히고
있던 결함**), 그것이 방어 지표의 분모와 값을 바꿨다.

| 지표 | 수정 전 | **수정 후(현재)** |
|---|---|---|
| 근거 정합률 | 0.9701 (681/702) | **1.0000 (681/681)** CI [1.0000, 1.0000] |
| 근거 초과율 | 0.0299 (21/702) | **0.0000 (0/681)** CI [0.0000, 0.0000] |
| 과잉 기권율 | 0.3500 (21/60) | **0.0000 (0/60)** CI [0.0000, 0.0000] |
| 스키마 준수율 | 1.0000 (60/60) | 1.0000 (60/60) CI [1.0000, 1.0000] |
| 적절한 기권율 | 측정 불가(분모 0) | 측정 불가(분모 0) |

★**이 문서가 "후속 조사 필요"로 남겨 뒀던 과잉 기권율 35% 의 정체가 이것이었다.**
모델이 과하게 기권한 것이 아니라, **선언 누락 때문에 제안이 기계적으로 막히고
있었다.** 지표가 결함을 가리키고 있었는데 그 방향으로 파고들지 않았던 것이다.

★부트스트랩 상한 인덱스의 off-by-one 도 함께 고쳤다(Codex 교차검증 지적) —
nearest-rank 는 `ceil(q*n)-1` 인데 상한만 `int(q*n)` 이었다.

### holdout 에서 방어 지표 5종을 재지 못하는 이유 (구조적)

DoD-28 의 남은 미충족분이라 채우려 했고, 원인 사슬을 끝까지 따라갔다.

```
holdout 24건은 expected_capability 라벨이 하나도 없다(golden 은 60/72 가 있다)
  → _seed_golden_fixtures 가 그 라벨로 배송 fixture 생성을 결정한다(common.py:113)
  → holdout 케이스에는 shipments 행이 안 생긴다
  → shipment.exception 이 조회할 배송 건이 없어 escalate 한다
  → 제안이 하나도 안 생긴다 (실측: 72행 전부, 재실행 후에도 0건)
  → 제안의 필드를 사실과 대조하는 방어 지표는 잴 대상이 없다
```

★**그리고 이 사슬을 다 고쳐도 의미 있는 수치가 안 나온다.** holdout 24건 중
`wait_for_approval` 을 기대하는 케이스가 **3건**뿐이다. 방어 지표를 n=3 으로
내는 것은 이 저장소가 금지하는 "작은 표본을 작다고 말하지 않는" 보고에 가깝다.

**결론: holdout 방어지표는 데이터셋을 승인 시나리오 중심으로 다시 짜지 않는 한
채울 수 없다.** holdout 은 프롬프트 수정에 쓰지 않는 동결 집합이라(v8 §15)
그 재설계는 별도 결정이다. 이 사실 자체를 미충족의 내용으로 적는다.
