...
2026-09-08 에 도메인이 **커머스(쇼핑몰) CS → 여행 CS** 로 교체됐고, 2026-09-10 에 **커머스 코드가 저장소에서 삭제됐다.**
...
| 문서 | 무엇이었나 | 코드 |
|---|---|---|
| `voc-store-manager.md` | 집계·급증 탐지. **v8 재판정으로 이미 껍데기였다** | 삭제됨 |
| `response-review.md` · `response-review-design.md` | 응답 생성과 검토를 다른 주체가 한다 | 삭제됨. **원칙은 여행 Team 내부 규칙으로 흡수**(v11 §5) |
| `return-refund.md` | 취소 기한·위약금율 판정. **Mock 유지로 결정됐다**(2026-09-06) | 삭제됨. 뼈대를 [activity](../../teams/activity.md) 가 물려받았다 |
| `fulfillment-logistics.md` | 구간·경로 판정. **`business_subject` 를 옳게 쓰는 유일한 Team 이었다** | 삭제됨. 뼈대를 [mobility](../../teams/mobility.md) 가 물려받았다 |
| `procurement-order.md` | 주문·금액·승인 검증 | 삭제됨 |
| `catalog-verification.md` | 외부 원장과 대조. **A2A Remote** | 삭제됨. 자리를 **Place Verification** 이 물려받았다 |
...
| 인용된 코드 경로는 대개 없다 | `app/modules/customer_ops/*` 는 2026-09-10 에 삭제됐다 |
