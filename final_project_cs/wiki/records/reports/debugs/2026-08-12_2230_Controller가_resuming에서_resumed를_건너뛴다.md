...
`CLAUDE.md` 상태표가 해결로 인용한다(Controller 행). 상태기계가 `resuming→completed` 를 런타임 거부해 잡아낸 진짜 결함이었고, `tests/contract/test_case_state_table.py` 가 전이표를 고정한다.

★표기만 2026-09-03 에 붙였다. 아래 `- 상태:` 줄은 작성 시점 기록이라
  지금은 사실이 아니다(`wiki/records/reports/2026-09-03_결함리포트_상태를_읽을_수_없다.md`).
