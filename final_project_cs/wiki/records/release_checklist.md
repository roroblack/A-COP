...
> 전체)의 현재 판정은 `wiki/records/evidence/DoD-*.md` 개별 파일이 정본이다 —
...
| | |
|---|---|
| DoD | evidence **29/29** · **통과 26 (90%)** · 부분통과 3 (10%) |
| | ★2026-09-07 정정 — 위 줄은 오래 **`18/18 · 통과 16 · 부분통과 2`** 였다. DoD 가 v7/v8 에서 18 → 29 로 늘었는데 이 요약만 안 따라왔다. `python -m scripts.verify_dod` 로 세어 갱신한다 |
| 게이트 | M1 · M2 · M3 **전부 도달** |
| ★**RC** | **아니다** — §5 의 차단 항목 참조 |
...
- [x] 4회 연속 동일 결과 — flaky 없음
  (★한 번 있었다: `observed_at` 이 시계에 걸렸다 → `wiki/records/reports/debugs/2026-08-14_시계에_걸린_테스트가_가끔_실패한다.md`)
...
- [x] ★**`unknown` 을 사람이 푸는 운영 절차(화면·런북)** — 2026-08-20 작성 완료.
      `wiki/records/manuals/운영_unknown상태_대응절차.md`. `unknown` 은 `customer_cases`
      ...
      근거: `wiki/records/reports/2026-08-20_S-OPS-UNKNOWN-RUNBOOK_리포트.md`.
...
- [x] 배포 후 워킹 트리 복구 확인
- [x] ★**배포용 선언 문제는 삭제로 해소됐다** — 아래 §4-1
      ★2026-09-07 정정: 이 줄은 `- [ ] 배포용 선언으로 바꾸지 않았다` 로
      **미체크인 채 남아 있었다.** 그런데 §4-1 의 두 항목은 둘 다 `[x]` 다 —
      `composer_ui` 를 토글로 끄는 대신 **통째로 지웠기** 때문에 배포 전에
      바꿀 선언 자체가 없어졌다. 부모만 옛 상태로 남아 있던 것이다.
...
- [x] ★**`composer_ui` 자체를 제거함(2026-08-18)** — 토글이 아니라 삭제다.
      ...
      전부 없앴다(`wiki/records/handoff/09`). 같은 기능은 `final_project_ui`의 인증된
      `/composer/*` 호출로만 제공한다.
...
v5 §15-4 는 judge 판정을 **사람 라벨**과 대조하라고 한다. **하지 않았다.**

★2026-09-07 실측 정정 — 계획서는 "20건" 이라 적지만 실제 템플릿
`eval/reports/holdout_human_labels_template.jsonl` 은 **24행**이다. 문서의 수를
옮겨 적지 말고 파일을 센다.
...
**필요한 것**: 템플릿 **24행**을 사람이 rubric 대로 채점 → judge 점수와 일치율·
Cohen's κ 산출. ★**나머지는 이미 준비돼 있다** — holdout 실행은 끝났고
(`candidate_answer`·`policy_evidence` 24/24 = 100% 채워짐), 합성 라벨로
`python -m eval.stats.agreement` 가 **끝까지 도는 것을 2026-09-07 에 확인했다**
(exit 0, 사람 24행 ↔ judged 72행 페어링 성공).

★**그러나 2026-09-09 사용자 결정으로 채점은 보류다.** 도메인이 여행 CS(v10)로
바뀌었고 이 24건은 **쇼핑몰(v9) 데이터**라, 채점해도 여행 제품의 judge agreement
를 말하지 못한다. **막는 것은 도구가 아니라 데이터의 도메인이다.**
DoD-15·17 은 부분통과인 채로 둔다 — 여행 golden/holdout 이 생기면 같은 도구로
다시 만든다(`wiki/records/TODO/보류_실비용_사람작업.md` 머리말).
...
- [x] ★**마우스 오버 하이라이트가 튀는 UI 버그 — 해소**(2026-09-02). 브라우저
      ...
      체크리스트만 갱신이 안 된 상태였다.
      근거: `wiki/records/reports/2026-09-02_S-UI-HOVER-JITTER_브라우저재현_리포트.md`
- [x] ★**커밋 ↔ Phase 자동 매핑 — 구현**(2026-09-02).
      ...
      경로→스트림→Phase 표의 출처는 `wiki/records/handoff/05_분업_규칙.md` 소유 표와
...
- [x] ★**스크린샷 증거 `wiki/records/screenshots/` — 채움**(2026-09-02, Playwright).
...
- [x] ★**실제 결제 provider 어댑터(mock) + timeout→unknown end-to-end 통합테스트**
      ...
      완주시켰다. 근거: `wiki/records/reports/2026-08-20_S-PAYMENT-TIMEOUT-MOCK_리포트.md`
