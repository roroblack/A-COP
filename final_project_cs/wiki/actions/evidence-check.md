...
| 대상 | 예 (여행) |
|---|---|
| 식별자 | 이 `booking_id` 가 **이 고객 범위에 실재하는가** — `[정정 2026-09-10]` 「이 여행 것인가」로 적었는데 코드는 Trip 소속을 보지 않는다. 사실은 tenant/customer 범위로만 조회된다(`app/core/verification.py:145-150`) |
| 금액 | 환급액이 **예약 금액**(`amount_cents`)을 넘는가 |
| 수량 | **일행 수**(`party_size`)가 정원(`capacity`)을 넘는가 |
| 소유권 | 이 **공급자 예약**(`supplier_bookings`)이 **이 고객 범위에 실재하는가** — `[정정 2026-09-10]` 「이 예약 것인가」는 과장이다. 두 키를 **따로** 실재 확인할 뿐 서로 짝인지는 안 본다. 같은 고객의 다른 예약과 공급자 예약을 짝지어도 걸리지 않는다 |
...
**셋 다 사람이 리뷰로 못 잡고 실행 중에 기계가 잡았다.** 상태 전이표와 append-only 이벤트 원칙([CLAUDE.md §0.3](../../CLAUDE.md))이 없었으면 조용히 넘어갔을 결함들이다.

### ④ 이 재검증 게이트가 반대 방향으로도 뚫렸다 — 정상 제안을 잘못 막았다

`[실측]` 2026-09-03. `calculation_basis` 필드를 선언 없이 `ignored`로 처리해 **모든 환불 제안이 이 재검증에서 "선언되지 않은 필드"로 기계적으로 막혔다.** DoD-28 방어지표의 과잉 기권율이 35%로 튄 원인이 여기 있었다 — 모델이 과하게 기권한 게 아니라 게이트가 정상 제안을 오탐했다. → [dod28-rerun.md](../../../wiki/evaluation/dod28-rerun.md)
...
**이게 근거 대조 코드에서 특히 나쁜 이유** — "거부해야 할 제안"이 서버 에러가 되면 `escalated`로도 못 가고 감사 로그도 안 남는다. [CLAUDE.md §0.1](../../CLAUDE.md)이 요구하는 "근거 없으면 답하지 않는다"가 **에러로 조용히 새는 경로**였다. 세 비정상 값 모두 크래시 대신 안전한 검증 실패로 처리되도록 고쳤다.
...
| 사람이 승인하면 | `[정정 2026-09-10]` **지금 코드에는 승인할 대상이 안 생긴다** — degraded 에서 제안이 오면 `action_requests` 를 만들지 않고 escalate 한다(`controller.py:309-311`, 바로 위 절과 같다). 아래는 원문이 적은 한계다 — **진행된다.** 금지되는 것은 "자동" 실행이다. 승인 화면에 degraded 배너가 뜨지만 **승인자가 그걸 읽었는지는 시스템이 모른다** → [approval.md](approval.md) |
