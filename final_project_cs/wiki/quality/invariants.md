...
`[실측]` 2026-09-03 신설. **결함을 고치면서 같이 걸었다.**
...
**이 페널티는 한 번도 적용된 적이 없었다.**

```python
if team_failed:          # :526  ← 여기서 정한 값을
    success = False
...
success = bool(judge["pass"])   # :537  ← 여기서 덮어썼다
```

인용 검증 결과로 rubric 을 다시 접는 블록이 `success` 를 무조건 다시 계산했다.

**순서가 전부인 종류의 결함이다.** 그래서 불변식이 "페널티가 있다"가 아니라 **"페널티가 나중에 걸린다"**다.

`[실측]` 테스트가 `_one()` 의 소스를 읽어 두 위치의 순서를 비교한다. **순서가 뒤집히면 실패한다.**

### 이 결함이 오래 산 이유

**인라인이라 테스트를 걸 수 없었다.** 순수 함수 `team_failed()` 로 뺐다.

> **틀려도 테스트가 초록으로 나오는 종류의 코드**였다. → [blind-spots.md](blind-spots.md)

`[미확보]` **결함 2(채점식이 승인 대기를 벌점 처리)는 아직 안 고쳤다.** 채점 기준을 바꾸는 일이라 판단이 필요하다. → [evidence.md](evidence.md)
...
`[실측]` **2026-09-01 기준 48개.** 양방향이 잠겼다.

```
코드 표식 48  ⊂  문서 불변식 51
코드에만 있는 ID   0개
문서에만 있는 ID   3개  ← INV-CS-TEAM-003·004·005
```

**남은 3개는 표식이 없는 게 맞다.** 판정이 `review`라 대응 테스트가 없다. 자동화되면 그때 표식이 붙는다.
...
```text
1. 문서의 automated 테스트 경로가 실재하는가      ✅ 117개 통과
2. 그 파일에 해당 test 함수가 실재하는가          ✅
3. 코드의 invariant ID가 카탈로그에 있는가        ✅ 0건 불일치
4. ID가 중복되지 않는가                          ✅
```

`[실측]` 검사기가 실제로 잡는지도 확인했다. 없는 파일·함수를 일부러 넣으니 2건 다 잡았다.
