...
``order_other``, ``shipping_delayed``, ``shipping_delivered_not_received``,
``shipping_other``, ``return_quantity_exceeded``, ``return_fee_or_period``,
``return_other``, ``exchange_stock_or_period``, ``exchange_other``, and ``other``.
A provider must return all four labels;
...
ISSUE_CODES = frozenset(
    {
        ...
        "exchange_stock_or_period", "exchange_other", "other",
    }
)
        ...
        response = OpenAI(api_key=settings.openai_api_key).chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": (
                    ...
                    "intent is order|shipping|return|exchange|other; sentiment is "
