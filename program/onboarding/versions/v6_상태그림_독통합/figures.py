...
#: 앞의 둘이 왼쪽 기둥, 뒤의 둘이 오른쪽 기둥이다. 원본 그림이 그렇게 세웠다.
#: 컴포넌트 아래에 모듈이 오는 것이 뜻이 있다. 못 빼는 것 아래 뺄 수 있는 것이다.
STRUCTURE_SPLIT = 2
STRUCTURE_FOOT = ("오른쪽 숫자는 몇 번 단계에서 쓰이는지다. "
...
LIFECYCLE_WAIT = [("waiting_input", "고객 답을 기다림"),
                  ...
                  ("resuming", "다시 이어서 실행")]
#: 전이 이름은 계획서 19절의 mermaid 원본에서 그대로 가져왔다. 지어내지 않는다.
#: 주 흐름 다섯 상태 사이의 네 전이.
LIFECYCLE_MAIN_EDGE = ["created", "classified", "team_task", "completed"]
#: LIFECYCLE_WAIT 의 앞 셋이 대기 상태이고 넷째 resuming 은 돌아가는 자리다.
LIFECYCLE_WAIT_SPLIT = 3
#: running 에서 대기 셋으로 나가는 전이.
LIFECYCLE_HOLD_EDGE = ["missing_input", "approval_required", "async_provider"]
#: 대기 셋에서 resuming 으로 돌아오는 전이.
LIFECYCLE_BACK_EDGE = ["valid_input", "approved", "callback"]
LIFECYCLE_RESUME_NOTE = ["resuming 은 resume_token 을 검증한 뒤에야",
                         "running 으로 돌아간다. 토큰은 한 번만 쓰고",
                         "24시간이 지나면 만료된다"]
LIFECYCLE_END_NOTE = ["자동 처리 한계에 걸리면 escalated 로 보내 사람이 받는다. "
                      "복구 불가는 failed, 취소는 cancelled 다.",
                      "셋 다 이 환불 건에서는 안 나왔다. "
                      "허용된 다음 상태 표에 없는 전이는 transition_case() 가 거부한다."]
LIFECYCLE_SRC = ("출처: program/plan/A-COP_구현계획서_v8.md 19절. "
                 "모든 전이는 case_events 에 추가만 되므로 나중에 그대로 재생할 수 있다")
...
CONTRACTS_TABLES_NOTE = ("customer_cases 는 case_events 를 순서대로 적용한 결과일 "
                         "뿐이다. 그래서 이벤트만 있으면 언제든 다시 만들 수 있다.")
CONTRACTS_WHY = ("계약이 왜 중요한가", [
    "Core 는 Team 안을 들여다보지 않는다. TeamManifest 와 execute() 두 가지만 쓴다. "
    "그래서 팀을 늘려도 Core 코드가 안 바뀐다.",
    "모든 계약은 정의되지 않은 필드를 거부한다. 오타로 만든 필드가 조용히 흘러 "
    "다니지 않는다.",
...
BRANCHES_FOOT = ("공통 원칙 하나다. 모르면 비워 두고 신호를 남긴다. "
                 "채워 넣거나 조용히 넘어가면 그 오류가 고객 답변까지 그대로 간다.")

#: 남는 표도 두 기둥이다. 왼쪽은 케이스 자체, 오른쪽은 실행 기록이다.
ARTIFACTS_SPLIT = 2
...
TRACEBACK = [
    ...
    (3, "API 응답을 본다  (제작 중)", "amber", False, [
        ...
        ("내용은 어디 있나", "case_events.payload_json 에 저장은 돼 있다. "
                       "응답이 그것을 안 꺼내 쓸 뿐이다"),
        ("그래서", "지금은 API 만으로 못 되짚는다. 1번이나 2번으로 한다"),
        ("제작 중", "payload_json 의 내용을 value 에 실어 내보내는 작업이 남아 있다. 끝나면 바깥 시스템도 되짚을 수 있다"),
    ]),
]
TRACEBACK_FOOT = ("1번과 2번은 지금 됩니다. 2번은 SQL 로 기계가 따라갈 수 있습니다. "
                  "3번은 제작 중이고, 그때까지는 1번이나 2번으로 확인합니다.")
