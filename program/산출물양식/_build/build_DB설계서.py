...
ENTITIES = [
    ...
    ("Primary Key", "project_configs", "deployment_id", "TEXT PK",
     "declaration(JSONB), revision, updated_at. 중앙 설정 저장소. 아래 6절 참고"),
]
...
d.insert_before_para("000000C6", d.build([
    ...
    ("b", "6. 중앙 설정 저장소 (설계 결정, 아직 연결되지 않음)"),
    "2026-08-29 에 설정을 어디에 둘지 결정했다. 정본은 "
    "program/plan/A-COP_Composer_중앙설정저장소_결정.md 다.",
    "지금까지는 대상마다 config/project.yaml 파일 하나를 두고 그 파일을 고쳤다. "
    "그런데 파일을 고치려면 고치는 코드가 대상 안에 있어야 하고, "
    "쓰기 가능한 구성 관리 기능이 고객 대면 프로세스에 있으면 보안 위험이 된다. "
    "대상이 수천 개면 관리할 때만 다시 넣는 방식도 성립하지 않는다.",
    "그래서 선언을 우리가 운영하는 중앙 저장소 한 곳에 두기로 했다. "
    "대상은 기동할 때 자기 선언을 읽기만 한다. 쓰기 코드가 필요 없다.",
    ("l", "테이블. project_configs. deployment_id 가 기본키이고 declaration 을 JSONB 로 담는다."),
    ("l", "deployment_id 는 배포 인스턴스를 가리키는 키다. 업무 개념인 tenant_id 와 일부러 분리한다. "
          "한 테넌트가 여러 배포를 가질 수 있기 때문이다."),
    ("l", "revision 은 선언 내용에서 계산한 값이다. 여기 같이 저장하는 이유는 조건부 쓰기에 쓰기 "
          "위해서이지 별도의 진실이 아니다. 둘이 어긋나면 declaration 이 맞다."),
    ("l", "동시성은 지금의 base_revision 낙관적 잠금을 그대로 쓴다. "
          "프로세스 안 잠금이 여러 인스턴스에서 깨지던 한계가 이걸로 해소된다."),
    ("b", "지금 어디까지 됐나"),
    "정직하게 적는다. 저장 계층의 골격만 있고 아직 연결되지 않았다.",
    ("l", "있는 것. acop_basement/core/config_store.py 에 ConfigStore 규약과 "
          "FileConfigStore, PostgresConfigStore 가 있다. 마이그레이션 006_project_configs.sql 도 있다."),
    ("l", "테스트. 통합 테스트 10건이 있고 통과한다. 조건부 쓰기가 낡은 revision 을 거부하는지, "
          "같은 base_revision 으로 둘이 쓸 때 하나만 이기는지, 없는 배포를 조용히 빈 값으로 "
          "돌려주지 않는지를 확인한다."),
    ("l", "없는 것. 이 모듈을 부르는 곳이 아직 없다. acop_composer 도 app 도 이 모듈을 쓰지 않는다. "
          "대상이 중앙에서 설정을 받아오는 경로도 아직 없다."),
    ("l", "정해지지 않은 것. 대상이 중앙에 못 붙을 때 마지막 설정으로 계속 돌지, 기동을 거부할지. "
          "설정 변경을 대상에 반영하는 방법. 파일 기반에서 옮기는 경로. "
          "이 작업을 이번 기간에 할지 이후로 미룰지."),
    ("l", "이 결정이 틀릴 수 있는 조건도 문서에 적혀 있다. 실제 배포 대상이 수십 개 이하로 "
          "확정되면 중앙 저장소의 운영 부담이 얻는 것보다 클 수 있다."),
    "저장에 성공해도 반영된 것은 아니다. 대상은 기동할 때 설정을 읽으므로 재시작이 필요하다. "
    "이건 파일이든 중앙 저장소든 같다.",
    ("b", "7. 백업과 보존"),
