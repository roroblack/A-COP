...
trip_watch_sweeper
  ...
  3  변화 없으면 → ★안내 시점이면 안내를 발행한다 (아래 §6-B)
                   → 다음 확인 시점을 갱신하고 끝
...
★**이동 시간은 이미 검증이 계산했다**(§5 코어 검증 층 — 이동 여유). **새로 만드는
계산이 아니라 있는 값을 역산에 쓰는 것이다.**
...
| | |
|---|---|
| **기계 원가** | **LLM 은 Case 1~9건 × 3~4원이라 여행 1건에 3~37원** `[추정]`. 분모는 §1 의 59,000원 — **0.1% 안쪽**. ★**합계는 아직 못 낸다** — Places·Routes 단가가 `[미확보]` 다 |
