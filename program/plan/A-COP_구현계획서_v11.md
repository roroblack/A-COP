...
| **가격** — 서울 1도시 상품의 값과 그 값을 확정·조정할 기준 | 계획 담당이 Codex 와 정한다 `[팀장 위임 2026-09-10]` | → §11-A |
...
| 2순위 | 가족·소그룹 |
| 판매 형태 | 팀당 여행 이용권. ★기준 상품 **7일·4인·서울 1도시** `[결정 2026-09-10 — 앞 판은 2도시]`. 가격은 §11-A |
...
`[미확보]` **주기를 정하는 것은 비용을 정하는 것이다.** 1단계의 기상·관광정보는
무료지만 Places·Routes 는 유료다(§11-A). **어느 소스를 어느 주기로 볼지**는 단가가
확인돼야 정해진다.
...
| | |
|---|---|
| **기계 원가** | **LLM 은 Case 1~9건 × 3.03~4.06원이라 여행 1건에 3~37원** `[추정]`. 분모는 §1 의 59,000원 — **0.1% 안쪽**. ★**합계는 아직 못 낸다** — Places·Routes 단가가 `[미확보]` 다 |
| **일정 안내의 원가** | ★`[2026-09-10 오후 정정]` **0이 아니다.** LLM 은 여전히 안 부르지만(Case 를 안 만들고 문구틀로 채운다), **출발 안내가 경로를 그때 조회한다**(§6-B ③) — **여행당 항목 수만큼 유료 호출**(7일 기준 20~40회 `[추정]`). Routes 단가가 `[미확보]` 라 금액은 아직 못 만든다. **초판의 「0에 가깝다」는 경로를 안 준다는 전제 위에 있었다** |
