...
| 알림 채널 | 적혀 있지 않았다 | **디스코드 웹훅 하나만.** 제품 경로는 에이전트 API | 7주에 세 채널을 만들 이유가 없다. 오류·재시도·중복은 채널마다 공짜가 아니다 |
...
| **알림 전달 계약** — 긴급 통지의 전달 기한 · 수신 확인 · 실패 시 처리 | **우리 혼자 못 정한다.** 외부 에이전트 쪽과 맞춰야 한다 | 통지 경로를 붙이기 전 |
| **Activity 쇼핑 제외 21종** — 시나리오에 넣은 대형마트가 그 목록 안에 있다 | 팀 | 2주차 |
| **무료·무예약 활동의 취소 판정 규칙** — 경복궁처럼 취소 기한·위약금이 없는 것 | 팀 (업무 규칙이라 코드가 못 만든다) | Activity 판정 규칙 코드화 전 |
| **강사가 말한 「에이전트」가 우리 Team 과 같은 것인가** | 팀 (확인) | 1주차 |
| 인바운드 전제의 손익 재계산 (해외 카드 수수료·환율·영어 채널 획득비) | 결제 수단 결정 뒤 | MVP 이후 |
| 59,000원 수용 여부 | 유료 파일럿 20~30팀 | MVP 이후 |

★**아홉 각각이 무엇을 놓고 갈리는지**는 [`A-COP_정할것_9건_설명.md`](A-COP_정할것_9건_설명.md)
가 풀어 적는다 — 선택지 · 고르면 무엇이 달라지나 · 안 정하면 무엇이 막히나.

★**여기 남긴 아홉은 혼자 정할 수 없는 것들이다.** 09-09~09-10 에 여행 전환 문서의
`[미확보]` 표시를 훑어 **실측으로 닫은 것 일곱 · 근거를 적어 결정한 것 여덟**을
§0-2A 로 올렸다.

★`[참고]` **닫은 수와 남은 수를 합한 분모가 문서마다 다르게 세어져 있다** —
같은 작업을 두고 「24 → 9」로 적은 곳과 「24 → 7」로 적은 곳이 있다. **분모가
확정되지 않았으므로 비율을 만들지 않는다.** 지금 확실한 것은 **남은 것이 이 표의
아홉**이라는 것이고, 그중 둘(손익·가격)은 MVP 밖의 사업 판단이다.
...
되고, DoD-8(거부하면 되돌린다)에서 **어디까지 되돌릴지**가 애매해진다.
...
`[미확보]` **분류는 확정이 아니다.** 쇼핑에서 제외한 **생활밀착형·저관광연관 21종**에
시나리오에 넣은 **대형마트**가 들어가 있다. 제외 목록을 다시 봐야 한다.
...
★`[결정 2026-09-10]` **디스코드 하나만 만든다.** 보내는 함수를 갈아 끼우면 된다고
해도 **오류·재시도·중복 처리까지 공짜는 아니다.** 7주에 세 채널을 만들 이유가 없다.

★**이건 제품 기본 채널이 아니라 시연 출력이다.** 제품 경로는 §1 이 못박은 대로
**고객의 개인 에이전트에게 API 로** 간다. 둘을 섞어 말하지 않는다.

★`[결정 2026-09-10]` **비밀은 `.env`, 나머지는 `config/`.** 이 저장소에 관례가 이미
있다. 웹훅 주소·토큰은 비밀이고, 어느 채널을 쓸지·재시도 횟수는 설정이다.

★**「보냈다」와 「여행자가 알았다」는 다르다.** 웹훅 성공은 인지가 아니다.
긴급 통지의 **전달 기한 · 수신 확인 · 실패 시 처리**는 `[미확보]` 이고,
**외부 에이전트 쪽과 맞춰야 하는 계약**이다(§0-3).
...
| 알림 채널 | **디스코드 웹훅 하나**(시연 출력). 제품 경로는 에이전트 API | 텔레그램·이메일 어댑터 |
...
★`[실측 2026-09-10]` **넷 중 둘이 이미 있다.** 운영 UI 라우트 여덟을 세었다 —
승인(쓰기 포함)과 추적이 실물이고, 목록 화면이 「사건」의 뼈대다. **새로 만들 것은
「일정」 하나다.** `[참고]` 그대로 쓸 수 있는지는 화면을 열어 확인해야 한다.

★**이 화면은 운영자용이다.** 고객 접점은 §1 대로 개인 에이전트이고 자체 앱을 만들지
않는다.
...
**24항목** — 자동 18 · 아키텍처 테스트 4 · 측정 2.
★v10 은 22항목이었다. 더한 둘은 23·24 다.
