...
★`[2026-09-10 오후]` **아홉 중 여덟이 닫혔다** → §0-4. 남은 것은 아래 셋이다.

| 항목 | 누가 정하나 | 언제까지 |
|---|---|---|
| **통지가 고객에게 닿았는지 어떻게 아나** — 채널은 정해졌다(§6-A). 남은 것은 **전달 기한 · 수신 확인 · 실패 시 처리**다. 웹훅이 200 을 돌려주는 것과 여행자가 본 것은 다르다 | 팀. **채널로 직접 보내므로 수신 확인은 채널이 주는 만큼만 된다** | 통지 경로를 붙일 때 |
...
★**둘째 줄은 Codex 검토가 찾았다.** §1 은 기준 상품을 7일·4인·**2도시** 59,000원으로
...
★**그래서 「닿았나」가 남는다.** 웹훅이 200 을 돌려주는 것과 여행자가 본 것은 다르고,
디스코드는 읽음을 알려주지 않는다. **전달 기한 · 수신 확인 · 실패 시 처리**는
§0-3 에 남겨 뒀다 — 채널이 정해졌다고 이것이 정해지지는 않는다.
