...
| 10 | **때가 되면 무엇을 하면 되는지도 알린다.** 변화가 없어도 보내는 **일정 안내**가 생긴다 — 알림이 두 종류가 된다 | §6-B (신설) · §0-1 |
...
§6-B 가 **Case 를 만들지 않는 것**과 **문구틀로 만드는 것**을 못박았다. 안 그러면
원가 계산(§11-A)이 흔들린다.
...
| **8** | **때가 되면 무엇을 하면 되는지 알린다** `[2026-09-10 오후]` | 안내 발행률 100%, 같은 시점 중복 발행 0건 |
...
### 6-B. 알림은 두 종류다 — 변경 통지와 일정 안내

`[결정 2026-09-10 오후]` 앞 판은 **변화가 있을 때만** 알렸다. **때가 되면 무엇을
하면 되는지도 알린다.**
...
| **LLM 을 부르나** | 부른다 (분류·대안 생성) | ★**안 부른다** — 저장된 값을 문구틀에 채운다 |
| **변화가 없어도 가나** | 안 간다 | **간다.** 그게 이것의 존재 이유다 |

★★**②가 Case 가 아닌 것이 이 설계의 핵심이다.** 안내는 **이미 검증을 통과해 저장된
일정을 읽어 시각을 알리는 것**이라 판정도 재계획도 없다. Case 로 만들면 7일 여행에
안내만 열몇 건이 쌓이고 **그만큼 LLM 을 부른다**(§11-A 의 원가 계산이 흔들린다).

#### 언제 보내나 — §9-A 가 이미 정한 세 시점을 그대로 쓴다
...
★**새 주기를 만들지 않았다.** 감시 루프가 이미 그 세 시점에 돈다(§6-A). **같은
바퀴에 얹는다.**
...
| 담는 것 | 어디서 오나 |
|---|---|
| 시작 시각 · 장소 | 최신 일정 버전 (§7-B `itinerary_items`) |
| **언제 나서야 하나** | **검증이 이미 계산한 이동 여유** (§5 코어 검증 층) |
| **취소 기한이 남았으면 그 시각** | Activity 판정이 아는 값 (§5) |
| 이 항목에 걸린 필수 조건 | `scope=item` 조건 (§7-B) |
| **여행계획서 링크** | 늘 붙인다 (§6-A) |

★★**§4-D 를 그대로 지킨다.** 안내는 **아는 것만 옮긴다.** "지금 3호선을 타세요"
같은 문장은 **그 순간 조회해야 나오는 것**이고, 우리는 안내를 만들려고 새로 조회하지
않는다. **모르는 것은 안 적는다** — 링크를 눌러 보게 한다.

★**출처와 확인 시각이 따라간다.** 이동 여유가 어제 계산한 것이면 **어제 것임을
적는다.** 안 적으면 안내가 조용히 §4-D 를 깬다.
...
| **일정 안내** `[2026-09-10 오후]` | **문구틀에 저장된 값을 채워 보낸다.** 세 시점(전날·아침·2시간 전). Case 를 만들지 않고 LLM 을 안 부른다 | 문장 다듬기(Composer), 새 조회, 미열람 재알림 |
...
| **일정 안내의 원가** | ★**0에 가깝다** — Case 를 안 만들고 문구틀로 채우므로 **LLM 을 안 부른다**(§6-B). 7일 여행에 안내가 열몇 건 나가도 원가가 안 는다. **문장을 LLM 으로 다듬기 시작하면 이 줄이 깨진다** |
...
| **26** | **정해진 시점이 오면 안내가 발행되고, 같은 시점에 두 번 발행되지 않는다** | 자동 |

★**26 은 되잡기 작업이 다시 돌아도 안내가 두 번 나가지 않는 것을 본다**(§6-B).
되잡기 작업은 **설계상 다시 돌 수 있어야 하므로**, 막을 자리는 실행이 아니라 발행이다.
