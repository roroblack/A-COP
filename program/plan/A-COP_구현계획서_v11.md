...
> **가고 싶은 곳과 예약을 주면 전체 여행이 성립하는지 검증하고, 여행이 끝날 때까지 상황 변화를 먼저 감지해 남은 일정을 조정하며, 때가 되면 무엇을 하면 되는지 알려주는 여행 CS 플랫폼.**

`[2026-09-10 오후]` 마지막 절을 더했다 — **일정 안내**(§6-B). 앞 판은 변화가 있을
때만 말을 걸었다. 안내는 **어디로 · 언제 나서나 · 어떻게 가나**까지 준다.
...
| v9 자산 | v10·v11 처리 | 근거 |
|---|---|---|
| Case 생명주기 12 상태 (**v9** §19) | **승계 + 확장** — 시스템 발기(發起) Case 를 위해 `trigger_source` 추가 | 아래 §4-B |
...
| Composer (응답 문구 소재) | **승계. 소재를 한국어 통지 문구로 교체**하고 **직접 경로용으로 고객 언어(기본 영어)를 더한다** `[2026-09-10 — 두 번 바뀌었다]` 초판 「영어」 → 「한국어 하나」 → **「한국어 원본 + 고객 언어」**(§0-4 결정 11) | §1 |
| 커머스 Team 4종 (Procurement+Order&Payment, Fulfillment&Logistics, Return&Refund, Catalog&Verification) | **MVP 경로에서 제외.** 등록은 남겨도 되나 호출되지 않는다 | v4 **§4-7** `[2026-09-10 정정]` 초판은 §3-7 이라 적었는데 그 절이 없다 |
...
| 화면 | 적혀 있지 않았다 | **다섯** — 운영자 넷(일정·사건·승인·추적) + **고객 하나**(여행계획서 링크) (§9-C) | 다섯 중 **둘이 실물, 하나가 뼈대, 둘을 새로 만든다** |
| DoD | 22항목 | **26항목** — 라우팅 도달성(23) · 제안 분리(24) · 링크가 늘 최신(25) · 안내 중복 0(26) | 23·24 는 **실제로 뚫려 있던 자리**, 25 는 **뺀 것의 대가**, 26 은 **는 기능**이다 |
...
| 항목 | 누가 정하나 | 언제까지 |
|---|---|---|
| ~~직접 경로의 언어~~ **닫힘** → §0-4 결정 11 | | |
| ~~기준 상품 2도시 대 MVP 1도시~~ **닫힘** → §0-4 결정 12 | | |
| ~~가격~~ **닫힘** → §0-4 결정 13 · §11-A | | |

★**세 줄 다 Codex 검토가 찾았다.** 첫 줄은 **오늘 오후에 접점을 둘로 바꾸면서
내가 만든 구멍**이다 — 접점이 API 하나일 때는 번역 주체가 에이전트로 분명했다.

★**둘째 줄에 대해.** §1 은 기준 상품을 7일·4인·**2도시** 59,000원으로
...
### 0-4. `[2026-09-10]` 결정 열셋

★**§0-3 의 아홉 중 여덟이 닫혔고**(1~8), **그 뒤 같은 날 넷이 더 정해졌다**(9~12).
★**11 은 2 를 직접 경로에 한해 뒤집는다** — 아래 ★.
각 결정이 본문 어디를 바꿨는지 같이 적는다.
...
| 11 | ★**알림 언어는 고객이 고른다.** 우리가 **번역해서** 보낸다. **기본은 한국어+영어.** 에이전트가 API 로 받아 가는 경로는 **에이전트가 알아서 번역**한다 | §1 · §5-A · §6-B · §7 · §9-A · §10 |
| 12 | ★**기준 상품은 서울 1도시다.** 다른 도시는 **중간발표 뒤 테스트 검증이 끝나면** 연다. **커버 못 하는 범위를 상품에 넣지 않는다** — 테스트할 수 없기 때문이다 | §1 · §9-A · §11-A |
| 13 | ★**가격은 팀당 49,000원(VAT 포함)이다.** `[팀장 위임 — 계획 담당이 Codex 와 정했다]` 2도시 59,000원에서 내렸다. **값이 흔들리는 조건과 바꾸는 기준을 같이 정했다** — 파일럿이 그 기준으로 판정한다 | §1 · §11-A |
| 10 | **때가 되면 무엇을 하면 되는지도 알린다.** ①변경(**생성 포함**) ②여행·하루 시작 ③**각 항목 출발 시각** 셋에 보내고, ③에는 **어디로·언제 나서나·어떻게 가나(경로)** 를 담는다 | §6-B (신설) · §0-1 |
...
★★**11번이 2번을 부분적으로 뒤집는다.** 2번은 "우리는 한국어 하나, 번역은 에이전트
몫"이었다. 그런데 **9번에서 접점이 둘이 되면서 직접 경로(링크·알림)엔 옮기는 주체가
없어졌고**, 11번이 그 구멍을 **우리가 번역하는 것**으로 메웠다.

| 경로 | 누가 옮기나 |
|---|---|
| **개인 에이전트 API** | **에이전트** — 2번 그대로 |
| **여행계획서 링크 · 알림** | ★**우리** — 고객이 고른 언어로. 기본 한국어·영어 |

★**그래서 번역 품질이 우리 몫이 된다** — 직접 경로에 한해. **"다국어 지원"이라고
말할 수 있는 범위가 생겼지만, 그 범위는 우리가 옮긴 두 경로뿐이다.**

★★**12번이 §0-3 의 모순을 없앤다.** 상품은 2도시인데 MVP 는 1도시였다. **파는 것을
만드는 범위에 맞췄다.** 2도시는 **확장 순서**이지 지금 파는 것이 아니다.

★★**10번이 제품 범위를 넓힌다.** 앞 판은 **변화가 있을 때만** 말을 걸었다. 이제
**때가 되면 말을 건다.** 그래서 §0-1 의 한 줄 정의에 한 마디를 더했다 — **정직하게
적는 편이 낫다.** 안내는 판정도 재계획도 없는 대신 **매일 여러 번 나가므로**,
§6-B 가 **Case 를 만들지 않는 것**과 **문구틀로 만드는 것**을 못박았다.

★★**「생성도 변경이다」가 빈 자리를 메운다.** 앞 판은 **여행계획서 링크를 언제
처음 주는지** 말하지 않았다. 최초 검증 통과가 ①에 들어가면서 **그 자리가 생겼다.**

★★**경로를 주기로 한 것이 원가를 바꾼다.** ③이 출발 시각에 **이동 시간 조회**를
부른다 — 여행당 항목 수만큼의 **유료 호출**이다. §11-A 의 「안내 원가는 0에 가깝다」를
정정했다. **LLM 은 여전히 안 부른다.**
...
| 2순위 | 가족·소그룹 |
| 판매 형태 | 팀당 여행 이용권. ★기준 상품 **최대 7일·4인·서울 1도시 — 팀당 49,000원(VAT 포함), 인당 12,250원** `[결정 2026-09-10]`. 앞 판 「2도시 59,000원」에서 바뀌었다 — 근거·상한·바꾸는 기준은 §11-A |
| **도시 확장** | **서울부터.** 다른 도시는 **중간발표 뒤 테스트 검증이 끝나면** 연다. ★**커버 못 하는 범위를 상품에 넣지 않는다** — 시나리오로 테스트할 수 없기 때문이다 |
...
| **우리가 내는 언어** | `[결정 2026-09-10 — 두 번 바뀌었다]` **경로마다 다르다.** ①**개인 에이전트 API** — 한국어로 내고 **에이전트가 옮긴다** ②★**여행계획서 링크·알림** — **고객이 고른 언어로 우리가 옮긴다. 기본은 한국어·영어.** 번역 품질은 ②에 한해 우리 몫이다 |
...
| **8** | **때가 되면 어디로·언제 나서야 하나·어떻게 가나를 알린다** `[2026-09-10 오후]` | 안내 발행률 100%, 같은 시점 중복 발행 0건, **출발 시각이 이동 시간에서 역산됐나** |

★**8 은 3 과 다르다.** 3 은 **변화가 있을 때** 말을 걸고, 8 은 **변화가 없어도**
때가 되면 말을 건다. 둘을 한 줄로 적으면 "변화가 없으면 조용한 제품"으로 읽힌다(§6-B).
...
★★`[실측 2026-09-10]` **이 게이트는 아직 없다.** `app/` 과 `tests/` 에서
`tier == 'simulated'` 를 세었고 **0건**이다. `travel_ops/booking_handoff.py:89~92` 는
**변경·취소를 전부 `WAIT_FOR_APPROVAL` 로 보낸다.**

★**지금은 그래서 안전하다** — 자동 실행 분기 자체가 없으니 실제 공급자에게 걸릴 것도
없다. **위험한 것은 시연을 위해 그 분기를 여는 순간**이고, **게이트와 아키텍처
테스트가 그보다 먼저 있어야 한다.** DoD-14·15 는 **아직 통과한 적이 없다.**
...
각 Team 은 셋을 갖는다 — ① 검증 규칙 ② **감시 소스 목록** ③ 재계획 후보 생성.

★`[2026-09-10 정정]` **②는 Team 의 능력이 아니라 「그 객체 종류에 무엇이 걸리나」의
목록이다.** **조회를 실행하는 것은 되잡기 작업**이고(§6-A), Team 은 그 목록을 선언만
한다. 초판은 "Team 이 감시 소스를 갖는다"로 읽혀 §6-A 와 어긋났다. **전체 일정 정합성(시간 충돌·예산·이동 여유)은 Team 이 아니라 코어 검증 층이 본다.**
...
| Composer 소재 | 한국어 CS 문구 | **한국어 통지 문구가 원본**이다. ★직접 경로(링크·알림)는 **고객 언어로 옮긴다 — 기본 영어**(§0-4 결정 11). 에이전트 API 경로는 한국어 그대로 |
...
★**상수만 바꾸면 부족하다.** `[실측 2026-09-10 오전]` 프롬프트에 커머스 어휘가
**따로** 박혀 있었다 — 문제 코드는 상수에서 만드는데 **요청 종류만 문자열 리터럴**이었다.

★★`[실측 2026-09-10 오후]` **그 자리는 고쳐졌다.** `travel_ops/feedback.py:86~96` 이
`INTENTS`·`ISSUE_CODES` 를 **둘 다 상수에서 뽑아** 프롬프트를 만든다. **남은 것은
어휘를 `config/` 로 빼는 것과 CI·기동 검사**이고, 그건 아직 안 됐다.
...
trip_watch_sweeper
  ...
  3  ★두 판정을 따로 한다 — 서로 배타가 아니다
       3a  변화가 있나   → 있으면 Case 를 만든다 (기존 경로 그대로)
       3b  안내 시점인가 → 맞으면 안내를 발행한다 (§6-B)
  4  다음 확인 시점을 갱신한다
```

★★`[2026-09-10 오후 정정]` **초판은 「변화 없으면 안내」로 적었다 — 틀렸다.**
그러면 **변화가 있는 날에는 안내가 안 나간다.** §3 목표 8 의 「안내 발행률 100%」와
바로 부딪힌다. **둘은 배타 조건이 아니라 각각 판정한다.**

★**둘 다 참이면 둘 다 나간다.** 다만 **변경 통지가 먼저 나가고, 그 변경이 안내
내용을 바꾸면 아직 안 나간 안내를 무효화하고 다시 계산한다**(§6-C-6).

★**4번이 핵심이다. 되잡기 작업은 Case 를 만들기만 한다.** 판정도 재계획도 하지
않는다 — 그건 기존 경로(분류 → Team → 재검증)가 이미 하는 일이다. **여기서 판정까지
하면 Team 을 두 벌 만들게 된다.**

★`[결정 2026-09-10 오후]` **3번 갈래가 늘었다.** 전에는 변화가 없으면 아무 일도
안 일어났는데, **때가 되면 무엇을 하면 되는지 알린다**(§6-B). **변화가 없어도 보낸다는
것이 앞 판과 다른 점**이다.
...
★`[2026-09-10 오후 정정]` **「각 예약 2시간 전」은 안내 시점이 아니다.** 감시가 도는
주기로는 그대로 두되, **안내를 보내는 시각은 이동 시간에서 역산한다**(§6-B). 고정값
두 시간은 걸어서 5분인 곳과 한 시간 반 걸리는 곳을 같게 취급한다.
...
이미 들어 있는 것 — 재시도 · 죽은 일꾼의 잠금 회수 · tenant 별 분리 실행 ·
**`UNIQUE(tenant_id, topic, dedupe_key)` 로 중복 발행을 DB 가 막는다**(§6).

★`[실측 2026-09-10]` **「재시도 3회」는 전부가 아니다.** `worker.py:50~57` 기준 —
**일반 예외는 최대 3회 뒤 `dead_letter`**, **타임아웃·연결 오류는 1회 뒤 `unknown`
이고 자동 재시도가 없다**(사람이 본다). 후자가 바로 **채널이 안 될 때 나는 오류**다.
...
| | 무엇인가 | 못 닿으면 |
|---|---|---|
| **알림** | 변화가 있었다는 **신호** | **우리 일정 버전은 이미 고쳐져 있다.** 아래 ★단서를 본다 |
...
★★`[2026-09-10 오후 정정]` **「고객이 틀린 일정을 들고 다니지 않는다」는 과했다.**
**우리 일정 버전이 맞는 것과 업체 예약이 맞는 것은 다르다**(§4-C). 항공편을 바꾸기로
정해 일정에 반영해도 **업체 예약은 승인 전이면 그대로**다. 그 상태로 계획서만 보면
**있지도 않은 항공편을 타러 간다.**

| 그래서 계획서가 **두 가지를 같이 보여준다** | |
|---|---|
| **우리 일정 버전** | 우리가 고친 것. 되돌릴 수 있다 |
| ★**「업체 변경 필요·미확정」** | **아직 예약이 안 바뀌었다.** 기존 확정 예약도 같이 보인다 |

★**「일정에 반영했다」가 「예약까지 확보했다」가 되면 안 된다.** 자세한 것은 §6-C-5.

#### ★무엇은 확인할 수 있고 무엇은 못 하나

`[결정 2026-09-10 오후]` **알림을 봤는지는 못 잰다. 계획서를 봤는지는 잰다.**

| | 알 수 있나 | 어떻게 |
|---|---|---|
| 알림을 **발행했나** | **된다** | 바깥함이 행으로 남긴다. 지우지 않는다 |
...
★★**마지막 줄이 답이다.** 못 재는 것(알림 열람)과 재야 하는 것(**고객이 최신 안을
봤나**)이 다르다. **제품이 약속하는 것은 뒤쪽**이고 그건 잴 수 있다 — DoD-25 가
그것을 판정한다.
...
`[미확보]` 열람이 오래 없을 때 다시 알릴지는 안 정했다. **다음 안내(§6-B)에
「아직 안 보신 변경이 있습니다」를 얹는 것**이 가장 싸 보이지만 확인하지 않았다.
...
`[결정 2026-09-10 오후]` 앞 판은 **변화가 있을 때만** 알렸다. **세 시점에 알린다.**
...
★★**①에 「생성」이 들어간다.** `[2026-09-10 오후]` 일정을 **처음 받아 검증을 통과한
것도 변경**이다. 그때가 **여행계획서 링크가 처음 나가는 자리**이기도 하다 — 링크를
언제 주는지 앞 판이 말하지 않았다.
...
| | **① 변경 통지** | **②·③ 일정 안내** |
|---|---|---|
| **왜 보내나** | 일정이 바뀌었다 | **지금 무엇을 하면 되나** |
| **Case 를 만드나** | **만든다** — 판정·재계획이 필요하다 | ★**안 만든다** — 판정할 것이 없다 |
| **LLM 을 부르나** | 부른다 (분류·대안 생성) | ★**안 부른다** — 문구틀에 값을 채운다 |
| **변화가 없어도 가나** | 안 간다 | **간다.** 그게 이것의 존재 이유다 |

★★**②·③이 Case 가 아닌 것이 이 설계의 핵심이다.** 안내는 **이미 검증을 통과해
저장된 일정을 읽어 알리는 것**이라 판정도 재계획도 없다. Case 로 만들면 7일 여행에
안내만 수십 건이 쌓이고 **그만큼 LLM 을 부른다**(§11-A 의 원가 계산이 흔들린다).
...
★`[2026-09-10 오후 정정]` 앞 판은 **「각 예약 2시간 전」** 이라고 적었다. 그건 v10 에서
온 **고정값**이고, **이동 시간이 항목마다 다르므로 고정값이면 매번 틀린다.**
걸어서 5분인 곳에 두 시간 전에 알리고, 한 시간 반 걸리는 곳에도 두 시간 전에 알린다.

★★`[실측 2026-09-10]` **「이동 시간은 이미 검증이 계산했다」고 적었는데 틀렸다.**
`app/tools/read_tools.py:252,256` 의 `read.route`·`read.transit` 는 **둘 다 항상
`None` 을 돌려준다** — 주석이 `[미구현]` 이라고 적어 뒀다. `mobility.py:51` 이
그것을 부르고 **「미확인」으로 끝난다.**

**그래서 역산은 지금 근거가 없다.** 순서가 이렇게 된다.

```
1  Routes 를 붙인다                    ← 이게 먼저다. 없으면 아래가 전부 못 돈다
2  검증이 이동 시간을 계산해 저장한다
3  그 값으로 출발 시각을 역산한다
```

★**그때까지는 안내를 「시작 시각 기준」으로만 보낼 수 있다.** 그건 이 절이 없애려던
바로 그것이므로, **Routes 연결이 §6-B 의 선행 조건**이다.

★**여유를 얼마로 두나는 설정이다.** 임계값은 `config/` 에 둔다(§6-A 와 같은 자리).

#### 무엇을 담나 — ★③에서는 새로 조회한다

★★`[2026-09-10 오후 정정]` **이 절의 초판은 "새로 조회하지 않는다"였다. 뒤집는다.**
*"지금 무엇을 하면 되나"* 를 알리면서 **어떻게 가는지를 안 주면 알림의 값이 없다.**

| 담는 것 | 어디서 오나 | 새로 조회하나 |
|---|---|---|
| 시작 시각 · 장소 | 최신 일정 버전 (§7-B `itinerary_items`) | 아니다 |
| **다음에 어디로 가나** | 다음 항목의 장소 | 아니다 |
| **언제 나서야 하나** | 위 역산 | 아니다 |
| ★**어떻게 가나 — 경로** | **이동 시간 조회 Action**(§5-A) | ★**③에서는 조회한다** |
| 취소 기한이 남았으면 그 시각 | Activity 판정이 아는 값 (§5) | 아니다 |
| 이 항목에 걸린 필수 조건 | `scope=item` 조건 (§7-B) | 아니다 |
| **여행계획서 링크** | 늘 붙인다 (§6-A) | — |
...
★★`[2026-09-10 오후 — 검토가 잡은 것]` **발행 시각을 낡은 값으로 정하고 최신 경로를
그 시각에 조회하면 이미 늦다.** 이동이 30분에서 60분으로 늘었으면 **조회 결과를 받는
순간 30분 늦은 것**이고, 알림은 정상 발송된 것처럼 보인다. **그게 이 설계에서 가장
위험한 자리다.**
...
| | 언제 | 무엇을 |
|---|---|---|
| **사전 조회** | 예상 출발 시각보다 **충분히 이르게** | 이동 시간을 갱신하고 **발행 시각을 다시 계산한다** |
| **출발 안내** | 다시 계산한 그 시각 | 경로를 조회해 담는다 |

★**「충분히 이르게」가 얼마인지는 안 정했다** `[미확보]` — 이동 시간의 변동폭을
봐야 정해진다. **너무 이르면 그 값도 낡고, 늦으면 재계산할 시간이 없다.**

★**호출 수가 통제된다.** 항목 하나에 한 번이므로 **여행 한 건에 항목 수만큼**이다
(7일 기준 20~40회 `[추정]`). 감시 주기마다 부르는 것이 아니다.

★★**§4-D 를 지키는 방법이 달라진다.** 조회하지 않는 것이 아니라 **조회한 것에
확인 시각을 붙이는 것**으로 지킨다.

| | |
|---|---|
| **조회가 되면** | 경로와 **조회 시각**을 같이 적는다 |
| **조회가 안 되면** | ★**「모름」으로 적고 링크로 보낸다.** 지어내지 않는다 |
| **저장된 값을 쓸 때** | 언제 계산한 것인지 적는다 — 어제 것이면 **어제 것임을 적는다** |

★**안내가 §4-D 를 깨는 가장 쉬운 길이 「그럴듯한 경로 문장」이다.** 실제로 안 본
환승 정보를 적으면 그 순간 이 제품이 파는 것의 반대가 된다.

#### ②는 언제인가

★**「여행계획서를 수행할 시간」은 여행 시작 시점이다.** 첫 항목의 **출발 시각**에서
역산하되, **전날 저녁에 한 번 더 보낸다** — 준비물·이동 수단은 당일 아침에 알면 늦다.

| | |
|---|---|
| **전날** | 내일 시작한다 · 첫 항목 시각과 장소 · 준비물 성격의 필수 조건 |
| **당일 시작 시각** | 오늘 항목들 · 오늘 걸리는 것(날씨·휴무) · 오늘 지나는 취소 기한 |

★**「매일 아침」은 ②가 매일 도는 것으로 읽는다.** 여행 둘째 날 아침도 *"오늘
계획서를 수행할 시간"* 이다.

#### 두 번 보내지 않는다

★**중복은 이미 DB 가 막는다** — 바깥함의 `UNIQUE(tenant_id, topic, dedupe_key)`
(§6). 안내의 `dedupe_key` 를 **여행 · 대상 · 안내 종류**로 만들면, 되잡기 작업이
같은 시점에 두 번 돌아도 **한 번만 나간다.**

★**이건 새로 만드는 안전장치가 아니라 있는 것을 쓰는 것이다.** 되잡기 작업은
설계상 다시 돌 수 있어야 하므로(그래서 "되잡기"다) **두 번 도는 것을 막는 것이
아니라 두 번 나가는 것을 막는다.**

#### 이것이 제품 정의를 넓힌다

★**정직하게 적는다.** §0-1 은 "변화를 먼저 감지해 조정한다"였다. 안내는 **변화가
없어도 보내는 것**이라 그 문장 밖에 있다. **지속 관리의 일부로 넣는 것이 맞다고
보지만, 범위가 늘어난 것은 사실이다.**

★**그리고 원가가 0이 아니게 됐다.** ③이 경로를 조회하므로 **여행당 20~40회의 유료
호출**이 붙는다(§11-A). 초판은 "안내 원가는 0에 가깝다"고 적었는데 **그 줄은
경로를 안 준다는 전제 위에 있었다.**

`[미확보]` **문구를 어디까지 만들지는 안 정했다.** 문구틀에 값만 채울지, Composer 가
문장을 다듬을지 — 후자면 LLM 이 붙고 위 표의 "LLM 을 안 부른다"가 깨진다.
**MVP 는 문구틀로 간다**는 것까지가 결정이다.

`[미확보]` **경로를 어느 수준으로 줄지 안 정했다.** 소요 시간만인지, 환승·노선까지인지.
**Routes API 가 무엇을 돌려주는지 확인한 뒤 정한다** — 확인 전에 문구를 설계하면
못 주는 것을 약속하게 된다.

### 6-C. 여러 곳이 한꺼번에 깨질 때 — 묶어서 하나로 고친다

`[결정 2026-09-10 오후]` **앞 판은 이 경우를 말하지 않았다.** "최고 안 하나를 적용한다"
고 세 곳에 적어 놓고 **「최고」가 무엇인지도, 여러 항목이 동시에 깨지면 어떻게 하는지도
없었다.**

```
한 스윕에서 동시에 잡힌다 —
  17:00 식당이 임시 휴업
  그 식당으로 가는 지하철 구간이 시위로 막힘
  14:00 액티비티가 우천 취소
```

★**셋은 얽혀 있다.** 액티비티를 옮기면 이동 시간이 달라지고, 그러면 식당 도착 시각이
달라지고, 지하철이 막혔으니 경로도 달라진다. **따로 고치면 서로를 깨뜨린다.**

#### 6-C-1. Case 는 **한 스윕·한 Trip 에 하나**다

| 안 | 이점 | 대가 |
|---|---|---|
| ★**Trip 당 묶음 Case 하나** | 같은 기준 일정으로 조합을 찾는다. **적용·재요청·통지 단위가 일치한다** | 여러 Team 을 부르는 실행 경로가 필요하다 |
| 사건별 Case 셋 | 사건 하나가 막혀도 나머지가 진행된다 | **셋이 서로를 모른다.** 같은 일정 버전을 밀치고, 고객은 세 번 통지받는다 |

★**§4-E 와 같은 논리다.** 거기서는 *"한 사건이 여럿을 흔들면 Case 를 쪼개지 않는다"*
였고, 여기서는 **서로 다른 사건 셋이 같은 Trip 을 흔든다.** 원인이 몇이든 **고쳐야 할
대상이 하나(일정 버전)면 Case 도 하나**다.

★**사건의 정체는 묶음 안에 각각 보존한다.** 통지에도 원인별 요약이 들어간다 —
"세 가지가 겹쳐 이렇게 바꿨습니다"가 되어야지 "일정이 바뀌었습니다"가 되면 안 된다.

#### 6-C-2. ★「최선」의 기준 — 탈락 먼저, 그다음 사전식 비교

★★**탈락 조건과 비교 기준을 섞으면 안 된다.** 벌점으로 두면 **다른 점수와 교환된다** —
"싸니까 필수 조건 위반을 상쇄한다"가 나온다.

**먼저 탈락시킨다. 순위는 살아남은 것끼리만 매긴다.**

| | 탈락 조건 | |
|---|---|---|
| **1** | **필수 조건 위반** | 시간 충돌 · 이동 여유 부족 · 예산 상한 초과 · 식단·인원 조건 위반. **필수 판정값이 미확인이어도 자동 적용에서 뺀다** |
| **2** | **잠긴 예약을 건드림** | 시간·장소·내용 변경, 삭제, 사실상 참석 불가능하게 만드는 변경 |
| **3** | **실행 권한·근거 부족** | 승인 없는 실제 업체 실행을 요구하거나, 근거의 실재·소유를 확인 못 함 |

**살아남은 것끼리 앞에서부터 비교한다 (사전식 — 앞이 갈리면 뒤로 안 뒤집는다).**

| | 비교 기준 | 방향 | |
|---|---|---|---|
| **1** | **바뀌는 항목 수** | 적을수록 | 값이 달라진 항목 id 를 센다. 한 항목에서 여러 칸이 바뀌어도 한 번 |
| **2** | **추가 비용** | 작을수록 | 이동비·새 예약비·취소 비용·기존 예약 잔여 부담의 합. **미확정 환급을 확정 할인처럼 빼지 않는다** |
| **3** | **원래 일정과의 시간 차이** | 작을수록 | 최대 이동량 → 합계 순 |
| **4** | **되돌리기 쉬운가** | 쉬울수록 | 내부 일정 복원만 → 외부 보상 필요 순. 같으면 복원 가능 기간이 긴 쪽 |
| 동률 | 정규화한 후보 식별자 | — | **같은 입력에 같은 결과**가 나오게 하는 장치다. 품질 점수가 아니다 |

★**이 순위는 「기존 계획을 덜 흔드는 쪽」을 고른다는 제품 선택이다.** 대가로 예산
상한 안에서 조금 더 비싼 안이 뽑힐 수 있다. **감당 못 할 비용은 순위를 바꿀 문제가
아니라 자동 조정 비용 상한으로 막는다.**

★**대체 항목은 원래 항목과의 대응 관계를 유지한다.** 안 그러면 기존 항목을 지우고
새 id 로 만들어 "바뀐 항목 수 0"을 만드는 편법이 통한다.

#### 6-C-3. 종류 순위는 **비교**가 아니라 **탐색 순서**에 쓴다

`[팀 논의 2026-09-10]` 팀장 안은 **장거리 차편 > 액티비티 = 숙박 > 요식 > 이동**이었다.

★**비교 기준으로는 안 쓴다.** 반례가 바로 나온다 — **무료 취소 항공편**과
**선결제 환불불가 액티비티**면 종류로는 항공이 위인데 **실제 손실은 액티비티가 크다.**
종류 순위가 담으려던 것(잠김·비용·대체 가능성)은 위 §6-C-2 에 **직접** 들어가 있다.

★★**대신 탐색 순서로 쓴다.** 탐색에 상한이 있으므로(아래) **어느 항목부터 후보를
고정하느냐가 결과를 바꾼다.** **자유도가 낮은 것부터 고정하면 빨리 수렴한다.**

```
1  장거리 차편(항공·기차)     대체 후보가 가장 적다
2  숙박                      그날 밤 잘 곳이다. 탐색이 오래 걸린다
3  액티비티                  시각이 고정돼 있으나 대체가 있다
4  식사                      가장 유연하다
─  이동                      ★위가 정해진 뒤 **계산한다**
```

★**이동을 순위에서 뺀 것은 팀장 판단이 맞다.** 이동은 **종속 변수**다 — 앞의 것들을
잇는 계산이라 독립적으로 고를 대상이 아니다. **다만 이동이 원인이 되어 앞을 흔드는
경우**(지하철 시위)는 있고, 그때 이동은 "고칠 대상"이 아니라 **입력이 바뀐 것**이다.

#### 6-C-4. 조합을 어떻게 찾나 — 상한 있는 탐색

★**§9-A 가 OR-Tools 최적화를 뺐으므로 규칙 기반이어야 한다.**

```
1  스냅샷을 고정한다        최신 일정·잠긴 예약·필수 조건·사건·근거를 한 묶음으로 읽는다
2  영향 범위를 넓힌다        깨진 항목 + 그것 때문에 흔들리는 앞뒤 이동·후속 항목
3  장소·시간 후보를 만든다   §6-C-3 순서로. 명백한 위반은 여기서 먼저 버린다
4  이동 후보를 만든다        ★3에서 고른 조합에 맞춰. 옛 경로를 재사용하지 않는다
5  ★일정 전체를 검증한다     각 Team 이 통과했다고 전체가 통과하는 것이 아니다
6  통과한 것끼리 §6-C-2 로 비교해 하나를 고른다   나머지는 재요청용으로 둘까지 보존
```

★**첫 통과 후보에서 멈추지 않는다.** 첫 번째는 **되는 안**이지 **최선**이 아니다.

`[추정]` **MVP 상한** — 잰 값이 아니라 구현·운영 상한이다.
...
| 단계별 후보 | 최대 3 |
| 완성 조합 검증 | 최대 81 |
| 외부 조회 | 묶음당 최대 24회, 같은 요청은 실행 안에서 캐시 |
| 탐색 시간 | 최대 120초. **실제 출발·취소 마감이 더 이르면 그쪽** |
| 버전 충돌 후 재계산 | 1회 |

★**"해가 없다"고 말하지 않는다.** 상한까지 봤는데 없으면 **「정한 범위에서 못 찾았다」**
다. 전역 최적을 찾았다고도 말하지 않는다.

#### 6-C-5. 못 풀면 — ★부분 반영하지 않는다

★★**"액티비티만 풀렸으니 그것만 먼저 적용"은 안 한다.** 활동 장소만 바꾸면 **아직 못
푼 이동·식당 도착 시각이 더 나빠질 수 있다.**

| 부분 반영을 허용하면 무엇이 깨지나 |
|---|
| 개별 수정은 맞아도 **저장된 전체 일정이 안 성립**할 수 있다 |
| 재요청 때 **어디까지 되돌릴지**를 다시 정의해야 한다 |
| 남은 문제를 풀 때 **이미 적용한 것을 되돌리는 탐색**이 추가된다 |
| "조정 완료" 통지와 실제 해결 범위가 어긋난다 |

★**그렇다고 옛 일정을 멀쩡한 것처럼 두지도 않는다.** 일정 내용은 유지하되 Trip 에
**재검토 필요 상태와 영향받은 항목**을 기록하고, **계획서 링크에도 휴업·취소·운행
중단 사실과 자동 조정 실패를 표시한다.** 이건 대체안의 부분 적용과 다르다.

사람에게는 **사건 셋 · 근거 · 시도한 조합 · 탈락 사유 · 미확인 값 · 마감 시각 ·
부분적으로 유효했던 후보**를 함께 넘긴다(DoD-10).

★★**「일정에 반영했다」가 「예약까지 확보했다」가 되면 안 된다.** 시간·이동·예산·필수
조건은 통과했는데 **업체 예약만 미확정**인 경우는 §4-C 가 허용한 경로다 — 그때는
**「업체 변경 필요·미확정」 상태와 변경 링크를 붙여** 조건부로 저장·통지한다.
**「예약 확정」 자체가 필수 조건이거나 금액·시각을 몰라 필수 조건을 계산 못 하면
자동 적용에서 탈락**한다(§6-C-2 탈락 1).

#### 6-C-6. 통지는 **적용된 일정 버전당 하나**

원인이 셋이어도 **바뀐 일정이 하나면 통지도 하나**다. 담는 것 —
원인 셋과 각각의 영향 · 최종적으로 바뀐 것 · **왜 이 안인가** · 추가 비용 ·
적용 안 한 통과 대안 요약 · 업체 변경 링크 · 계획서 링크.

★**§6-B 의 하루 시작·출발 안내는 별개다.** "세 변화에 통지 하나"이지 "그날 알림을
하나로 제한한다"가 아니다.

★★**일정이 바뀌면 아직 안 나간 출발 안내를 무효화하고 다시 계산한다.** 안 그러면
**낡은 출발 시각으로 안내가 나간다.**

★**저장과 발행을 같은 트랜잭션에 넣는다** — 일정 버전 append · 최신 버전 포인터 ·
Case 전이 · 바깥함 삽입을 함께 커밋한다. 실패하면 전부 롤백. `transition_case()` 가
이미 그 모양이다(호출자가 트랜잭션을 소유한다).

#### 6-C-7. Trip 의 동시성 — Case 잠금으로는 안 된다

★★**지금 낙관적 잠금이 지키는 것은 Case 한 행이다.** `customer_cases` 를
`WHERE version = expected_version` 으로 막는다. **Case 가 서로 다르면 같은 Trip 이어도
안 부딪힌다.** Trip 의 일정 버전을 보호하는 장치는 **아직 없다.**

**최소 계약 다섯**

```
1  기준 일정 버전 + 사건 입력 개정번호를 함께 읽는다
2  후보를 계산하는 동안 긴 트랜잭션을 쥐지 않는다
3  반영 직전 둘이 그대로인지 확인하고 SQL 조건으로 다시 강제한다
4  달라졌으면 후보를 버리고 최신 입력으로 한 번 다시 계산한다
   ★버전 숫자만 바꿔 같은 후보를 밀어 넣지 않는다
5  성공할 때만 일정·포인터·Case·바깥함을 함께 커밋한다
```

★**개정번호까지 보는 이유** — **일정 버전이 그대로여도 새 운행 중단이나 예약 잠금
변경이 들어올 수 있다.** 일정 버전만 확인하면 **근거가 낡은 채로 반영된다.**

★★`[문서 내부 충돌 — 2026-09-10 해소]` **§9-A 가 「동시 수정 충돌 처리」를 범위에서
뺐는데, 공유 Trip 을 자동으로 고치면서 충돌 방지까지 뺄 수는 없다.**
**버전 조건으로 낡은 쓰기를 거부하고 한 번 재계산하는 것까지는 MVP 에 넣는다.**
빼는 것은 **다중 편집 병합과 충돌 해결 화면**이다.

#### 6-C-8. `[실측 2026-09-10]` 지금 코드로는 안 된다 — 세 자리

| 무엇 | 어디 | 왜 문제인가 |
|---|---|---|
| ★**대상을 안 가린다** | `app/tools/read_tools.py:154~162` | `read.booking` 은 `booking_id` 가 없으면 **`ORDER BY starts_at ASC LIMIT 1`** 이다. `activity.py:66` 은 `case_id` 를 넘기는데 **그 인자는 쓰이지 않는다.** Case 를 셋으로 나눠도 **셋이 같은 예약을 읽는다** |
| ★★**게다가 「임박한」도 아니다** | 같은 자리 | `[정정 2026-09-10 오후]` 코드 주석과 이 계획서 초판이 **「가장 임박한 예약」** 이라고 적었는데 **미래 시각 조건도 취소 제외도 없다.** 실제로는 **시작 시각이 가장 이른 예약**이고 **끝난 여행·취소된 예약도 나온다** |
| ★**시각 타당성을 사람이 본다고 돼 있다** | `travel_ops/verification_policy.py:70` | `starts_at_proposed` 가 대조 제외이고 주석이 *"승인자가 사람 눈으로 본다"* 다. **승인 없이 자동 적용하려면 이 가정을 없애고 시각 타당성을 코드가 판정해야 한다** |
| **감시가 Case 를 안 만든다** | `travel_ops/watch.py` | 관측과 `watch_changes` 저장까지 한다. **거기서 Case 로 넘어가는 경로가 없다** — §6-A 가 말한 자리가 아직 비어 있다 |

★**첫 줄이 가장 위험하다.** §5-D 에 *"예약이 없으면 「모름」으로 끝난다"* 고 적었는데
**그보다 나쁘다 — 엉뚱한 예약을 읽고 판정까지 할 수 있다.**
...
★**팀 보유 자산 중 쓰이는 것:** 커머스 주문 기록(`datasets/commerce/*`)은 이 도메인에서 쓰지 않는다.

★`[2026-09-10 — 같은 날 두 번 바뀌었다]` **번역 성능 비교 자료(`datasets/mt/*`)가
제품 경로로 돌아온다.** 오전엔 "우리 출력이 한국어 하나라 번역이 우리 층에 없다"로
빼냈는데, **결정 11 로 직접 경로(링크·알림)를 우리가 옮기게 됐다.** 번역 모델을
고를 때 이 자료가 **바로 쓰인다** — 기본 언어가 영어이므로 **한→영 품질이 첫 대상**이다.
...
bookings             예약. 잠긴 것과 안 잠긴 것          ← 010 에 이미 있다
watch_schedule       다음 확인 시점 — 감시 루프가 읽는다
```

#### ★감시는 상태를 가져야 한다

`[실측 2026-09-10]` **이 부분은 이미 만들어졌다** — `012_watch_observations.sql`.
계획서가 코드보다 늦었다.

```
watch_observations   매 폴링의 관측 + ★fingerprint (판정에 쓰는 부분만 해시)
watch_changes        fingerprint 가 달라진 것만. before/after/summary/affected_bookings
```

★★**무상태 폴링으로는 「바뀌었다」를 판정할 수 없다.** 직전 관측이 없으면 기준이
없어서 **매번 전부를 변화로 보거나 아무것도 못 본다.** 그래서 **관측을 남기는 것이
감시의 전제**다.

★**fingerprint 를 쓰는 이유** — 응답 전체를 해시하면 **공급자가 판정과 무관한 필드를
건드릴 때마다 오탐**이 난다. 판정에 쓰는 부분만 뽑아 해시한다.

★**관측과 변화를 다른 표에 두는 이유** — 관측은 2분마다 쌓이고 변화는 드물다.
한 표면 "무엇이 바뀌었나"를 찾을 때마다 **관측 더미를 뒤지게 된다.**

★**무엇을 그 주기로 보나** — 카탈로그 전체가 아니라 **고객 계획서에 실제로 걸린
대상**만 본다(§7-C). 감시 대상은 예약에서 나온다.
...
★★`[결정 2026-09-10 오후]` **주기는 분 단위다 — 최소 84초, 기본 2분.** 문제가 생겼을
때 **고객이 빨리 알고 대응할 수 있어야** 하므로 하루 몇 회로는 부족하다.

★**「거의 안 바뀌니 자주 안 봐도 된다」는 예산 논리이지 제품 요구가 아니다.**
드문 변화일수록 **아무도 안 보고 있다가 그 한 번이 나면 여행이 망한다.**

★**그래서 「무엇을」 보느냐로 비용을 줄인다.** 카탈로그 전체를 2분마다 볼 필요는
없다 — **고객 계획서에 걸린 대상**만 보면 되고 그건 훨씬 적다. **주기를 늘려 줄이지
않고 대상을 좁혀 줄인다.**

★★`[결정 2026-09-10]` **2분 주기는 무료 소스에만 쓴다.** 단가가 확인됐다(§11-A) —
Google Places 를 2분마다 부르면 **장소 한 곳에 7일 151,200원, 상품가의 3.1배**다.

| 소스 | 과금 | 주기 |
|---|---|---|
| 기상 · 관광정보(TourAPI) · 공휴일 · 국가유산 | 무료 (공공데이터는 **일일 한도**가 있다) | **2분** — 틱당 5건씩 교대 |
| Google Places · Routes | 유료 | **저빈도·캐시.** 출발 안내의 경로 조회처럼 **필요한 시점에만** |

`[실측]` 지금 코드가 이미 이렇다 — 장소 슬롯이 `TourApiPlace` 이고 틱당 5건이다.
★**2분 × 5건 = 하루 3,600건** 수요다. 공공데이터포털 키의 일일 한도와 겹치므로
**소스들이 한 `limiter` 를 공유하는 것**이 맞다(`base.py` 가 이미 그렇게 적어 뒀다).
한도 수치 자체는 `[미확보]` — 키 등급마다 다르다.
...
★`[2026-09-10 정정]` **초판은 단계 1 을 「계획과 검증」이라고만 불렀는데 §9-A 의
범위에는 감시·통지·안내·조정이 다 들어 있다.** 이름이 범위를 덜 말하고 있었다 —
**「지속 관리」가 이 제품의 정체**인데(§3 목표 3) 단계 이름에서 빠져 있었다.

| 단계 | 범위 | 이 프로젝트에서 |
|---|---|---|
| **1. 계획·검증·지속관리** `[2026-09-10 정정]` | 일정 수신·저장, 예약·이동·예산 검사, 출처·확인 시각 표시, 에이전트 API, **감시·조정·통지 루프**(§6-A~§6-C) | **MVP — 여기까지 한다** |
...
★`[2026-09-10 정정]` **초판 제목은 「7주」였다.** 그건 **09-08 착수 기준**이고 §0 은
남은 기간을 **6주 4일**(09-10 기준)로 적는다. **범위는 그대로 두고 기간 표기만
착수일에 맞춘다** — 아래 §9-B 의 주차 배분이 09-08 기준 7주다.

| 항목 | 최소 범위 | 이번엔 뺀다 |
|---|---|---|
| 고객·지역 | 인바운드 외국인 개인·친구 그룹, **서울** `[결정 2026-09-10]` — **기준 상품도 서울 1도시**(§1) | 다도시(**중간발표 뒤 검증이 끝나면 연다**), 가족, 한국인 해외여행 |
| 언어 | **원본은 한국어.** 직접 경로(링크·알림)는 **고객이 고른 언어로 옮긴다 — MVP 는 한국어·영어 둘.** 에이전트 API 경로는 한국어 그대로 | **한국어·영어 밖의 언어**(설정 자리는 두되 번역 품질을 검증하지 않는다), 에이전트 경로의 번역 |
| 계획 생성 | **만들지 않는다** | 자체 추천·생성기 |
| 검증 API | 엔드포인트 하나 — 코드 검증 / 확인 수준·시각 / 불가능 사유·완화 조건. 결과는 `ActionProposal` | OR-Tools 최적화, 보행·피로 모델 |
| 상태 저장 | Trip 1건 = 최신 일정 버전 + 잠긴 예약 + 필수 조건 + 다음 확인 시점 + **통지·재요청 이력** + **제시했던 다른 안** | 백업·복구, **다중 편집 병합·충돌 해결 화면** |
| **Trip 동시성** `[2026-09-10 오후]` | ★**버전 조건으로 낡은 쓰기를 거부하고 한 번 재계산한다**(§6-C-7). 초판은 「동시 수정 충돌 처리」를 통째로 뺐는데, **공유 Trip 을 자동으로 고치면서 충돌 방지까지 뺄 수는 없다** | 병합·해결 UI |
| **묶음 조정** `[2026-09-10 오후]` | **한 스윕·한 Trip = Case 하나.** 상한 있는 탐색으로 조합을 찾고 사전식 순위로 하나를 고른다(§6-C) | OR-Tools 최적화, 전역 최적, 부분 반영 |
| 지속 관리 루프 | **감시 루프 하나 + 배달 루프 하나**(§6-A). 로컬 주기 실행. **안내를 보내는 시각은 주기가 아니라 역산으로 정한다**(§6-B) | 업체 웹훅, 클라우드 스케줄러 |
| **감시 주기** `[결정 2026-09-10 오후]` | ★**분 단위다 — 최소 84초, 기본 2분.** 문제가 생겼을 때 고객이 빨리 알고 대응할 수 있어야 하므로 하루 몇 회로는 부족하다. **무엇을 그 주기로 볼지는 §7-C 가 가른다** — 카탈로그 전체가 아니라 **고객 계획서에 실제로 걸린 대상**만 본다 | 초 단위 실시간, 카탈로그 전수 폴링 |
...
| **일정 안내** `[2026-09-10 오후]` | **문구틀에 값을 채워 보낸다.** ①변경(생성 포함) ②여행·하루 시작 ③**각 항목 출발 시각**(이동 시간에서 역산). ③은 **경로를 그때 조회한다** — 어디로·언제 나서나·어떻게 가나. Case 를 만들지 않고 LLM 을 안 부른다 | 문장 다듬기(Composer), ①·②에서의 새 조회, 미열람 재알림 |
| 인프라 | **로컬 실행 + 외부 LLM API** | Cloud Run·Supabase 배포 |

### 9-B. 7주 배분 `[추정]` — **09-08 착수 기준**
...
★★`[2026-09-10 — 검토가 잡은 것]` **이 표에 안 들어간 필수 작업이 여럿이다.**
Booking Handoff(§5 에서 MVP 필수) · 배달 루프와 디스코드 어댑터(§6-A) ·
**여행계획서 링크**(§9-C) · **일정 안내**(§6-B) · **묶음 조정**(§6-C) · 화면.

★**주차를 지금 새로 배분하지 않는다.** 위 항목 대부분이 오늘 정해졌고, **1주차 실측
전에 다시 추정하면 추정 위에 추정을 쌓는 것**이다. **다만 빠져 있다는 사실은 적어
둔다** — 이 표를 일정 근거로 쓰면 안 된다.

★**절삭 순서는 §10 이 이미 정했다** — Activity 하나만으로도 루프가 성립하게 설계하고
Dining·Mobility 를 뺄 수 있게 둔다.

### 9-C. 화면은 다섯이다 — 운영자 넷 + 고객 하나
...
| **직접 경로의 번역 품질** | `[2026-09-10]` 결정 11 로 링크·알림을 **우리가 옮긴다.** 틀린 번역은 **틀린 시각·틀린 장소**로 이어진다 — 판정이 맞아도 고객이 잘못 읽는다 | ★**번역하지 않을 칸을 정한다** — 시각·날짜·금액·장소 고유명사·예약번호는 **원문 값을 그대로 싣는다.** 문장만 옮긴다. MVP 는 한·영 둘만 검증한다 |
...
| | |
|---|---|
| ★**가격** `[결정 2026-09-10 — 팀장 위임, 계획 담당·Codex]` | **팀당 49,000원(VAT 포함) — 서울·최대 7일·4인.** 인당 12,250원. 앞 판 59,000원은 **2도시 값**이라 내렸다 — **도시 간 연결을 관리하는 가치가 빠졌다.** 기간 중 검증·조정·현지 대응은 그대로다. 근거·상한·바꾸는 기준은 아래 행과 [`A-COP_비용과_수익구조.md`](A-COP_비용과_수익구조.md) §4-A |
| **기계 원가** | **LLM 은 Case 1~9건 × 3.03~4.06원이라 여행 1건에 3~37원** `[추정]`. 분모는 §1 의 59,000원 — **0.1% 안쪽**. ★**합계는 아직 못 낸다** — `[2026-09-10 정정]` Places·Routes **단가는 공개돼 있다**(v3 §7: Place Details $20/1,000건 · Route Matrix $5/1,000요소 `[외부]`). **모르는 것은 사용량이다** |
| **일정 안내의 원가** | LLM 은 안 부른다(Case 를 안 만들고 문구틀로 채운다). **출발 안내의 경로 조회는 1회 = 1요소**라 20~40회 × $5/1,000 = **150~300원/여행** `[추정]`. **작다** |
| ★★**2분 폴링은 무료 소스에만** | `[실측 2026-09-10]` 지금 감시의 장소 소스는 **`TourApiPlace`**(`infrastructure/travel/base.py`)이고 **틱당 5건**(`watch.py DEFAULT_BATCH`)이라 **비용이 안 붙는다.** ★**여기에 Google Places 를 붙이면 상품이 무너진다** — 2분 × 7일 = 5,040회/장소 × $20/1,000 = **$101 ≈ 151,200원, 장소 한 곳에 상품가의 3.1배.** 유료 소스는 **저빈도·캐시**로만 부른다(§7-C) |
| ★**사람 손 대 기계** | **비슷한 크기다.** 인계 15분 **7,500원**(분당 500원 완전원가 — 검토서 §7) 대 기계 **≈ 9,500원**(외부 데이터 8,000 + LLM 1,500) — **약 0.8배.** ★`[2026-09-10 정정 — 두 번]` 초판은 「사람 손이 전부를 정한다, 기계 원가 전체의 70~860배」였다. **분모가 LLM 뿐이었다** — 단가를 넣으니 뒤집혔다 |
| **그래서** | **둘 다 상한으로 묶는다** — 기계 10,000원 · 사람 1건·15분(아래 「운영 상한」). 인계 비율과 실제 호출량은 `[미확보]` — 파일럿에서 잴 첫 번째 수 |
| **자체호스팅** | **MVP 는 API 경유다** — §9-A 의 범위 결정이다. ★`[2026-09-10 정정]` 초판은 "12GB 에 안 들어간다 `[실측]`"이라 적었는데 **잰 것은 커머스 학습 OOM** 이다. 추론·여행 입력은 `[미확보]` |
| **응답 지연** | 커머스 실측 20~34초로 **실시간 채팅 제약은 사라진다** — 비동기 통지다(§6-A). `[판단]` **출발 직전 처리의 허용 지연은 안 쟀다** |
| **수익 구조** | **여행자 단품 49,000원이 기본이다.** 에이전트 사업자 구독+종량은 **후보로 남긴다** — ★`[2026-09-10 정정]` 초판은 "접점이 이미 에이전트"를 근거로 이것을 권했는데 **접점은 둘**이고(§1) **사업자의 지불 유인은 `[미확보]`** 다 |
| **운영 상한** `[추정 — 설정]` | 여행당 **기계 10,000원** · 사람 검토 **1건·15분** · 자동 조정 사건당 추가 호출 500원 · 같은 실패 재시도 1회 · **고객 추가 여행비 기본 0원**(고객이 따로 정한다). **초과하면** 선택적 탐색을 멈추고 미해결 이유·마지막 확인 시각·업체 링크를 준다. ★**기본 관리를 못 지키면 고지하고 전액 환불 선택권을 준다.** 필수 감시를 몰래 줄여 상한을 맞추지 않는다 |
| **기여이익** `[추정 — 예산 시나리오]` | 49,000 ÷ 1.1 × (1−3% 환불) = **43,209원** − (기계 10,000 + 사람 7,500 + 장애 1,000 + 획득 8,500 + 결제 4% 1,960) = **팀당 14,249원.** ★**고정비(개발·서버·번역 문구 제작)를 안 뺐다 — 순이익이 아니다** |
| ★**가격을 바꾸는 기준** `[추정]` | ①**유료 전환율** 10% 이상이면 59,000원 검증, 5% 미만이고 주된 거절이 가격이면 39,000원 검증(39,000원이면 기여이익 5,831원이라 **지금 가정으로는 더 안 내린다**) ②**팀당 실현 절감 가치** 중앙값 80,000원 이상 인상 · 39,000원 미만 인하·재구성 ③**평균 기여이익** 12,000원 목표 ④**상한 때문에 못 끝낸 여행** 10% 초과면 재설계. ★**MVP 엔 결제가 없어 ①을 못 잰다** — ②는 시나리오 대리지표로, ④는 비용 계수·검토 타이머를 넣으면 시험 입력에서 잰다 |
| ★**넣으면 안 되는 것** | **판정에 개입하는 광고·제휴.** ★`[2026-09-10 정정]` §4-D 와의 **명문 충돌이 아니라 제품 신뢰와의 긴장**이다. 넣으려면 ①판정과 분리 ②표시 ③거부해도 대안이 남을 것 + ④후보 생성은 비상업 기준 ⑤같은 재검증 ⑥개입 감사 ⑦제휴 끄기 — **일곱을 다 지켜야 한다** |
...
**26항목** — 자동 20 · 아키텍처 테스트 4 · 측정 2.
★v10 은 22항목이었다. v11 이 더한 넷은 23·24·25·26 이다.
...
★★**§6-C(묶음 조정)는 DoD 를 새로 안 만든다. 있는 것을 강하게 읽는다.**

| DoD | §6-C 에서 무엇을 확인하나 |
|---|---|
| **1·6** | 한 스윕의 사건 셋이 **Trip 당 Case 하나**로 묶이고, 적용하면 **일정 버전이 하나만** 는다 |
| **2·3·7·22** | Team 별 통과 후보를 **조합했을 때** 생기는 충돌을 최종 검증이 잡는다. **필수 조건 위반은 점수가 좋아도 탈락**한다 |
| **8·9** | 묶음 단위로 재요청을 처리한다. ★**휴업 식당이 든 옛 버전을 사실 확인 없이 복원하지 않는다** |
| **10** | **해 없음 · 근거 부족 · 탐색 상한 도달**을 구분해 넘긴다. 실패 시 **부분 적용이 없다** |
| **12·24** | 사건을 다시 수집해도 이중 반영이 없고, 대상이 다른 작업은 각각 저장된다 |
| **25·26** | 계획서가 **미확정 업체 건과 재검토 필요 상태**를 보여준다. **일정이 바뀌면 낡은 출발 안내가 안 나간다** |

★**DoD-7(재검증 통과 뒤에만 통지)은 「조정 성공 통지」에 걸리는 조건이다.**
안 그러면 **자동 조정에 실패했을 때 휴업·취소 사실조차 못 알린다.**

★**26 은 두 가지를 같이 본다**(§6-B) — **안내에 목적지·출발 시각·경로가 들어 있나**,
그리고 **되잡기 작업이 다시 돌아도 두 번 나가지 않나.** 되잡기 작업은 **설계상 다시
돌 수 있어야 하므로**, 막을 자리는 실행이 아니라 발행이다.

★**경로를 못 얻었을 때는 「모름」이 맞는 답이다**(§4-D). 그건 DoD-5 가 이미 본다 —
**26 은 「경로 칸이 있나」를 보지 「경로가 늘 있나」를 보지 않는다.**
