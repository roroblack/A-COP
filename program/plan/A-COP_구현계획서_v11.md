...
| 항목 | 누가 정하나 | 언제까지 |
|---|---|---|
| ★**직접 경로의 언어** — 1순위 고객이 **인바운드 외국인**인데 우리 출력은 **한국어 하나**다. 에이전트가 옮긴다는 전제인데 **여행계획서 링크와 알림은 고객에게 직접 간다**(§1) — **그 경로엔 옮기는 주체가 없다.** ①알림을 에이전트 경유로 ②직접 경로만 다국어 ③인바운드 약속을 줄인다 중 하나 | 팀 | **링크·알림을 붙이기 전** |
...
### 0-4. `[2026-09-10]` 팀장 결정 열

★**§0-3 의 아홉 중 여덟이 닫혔고**(1~8), **그 뒤 같은 날 둘이 더 정해졌다**(9·10).
...
| 무엇 | 어디 | 왜 문제인가 |
|---|---|---|
| ★**대상을 안 가린다** | `app/tools/read_tools.py:143~162` | `read.booking` 은 **`booking_id` 가 없으면 「가장 임박한 예약」 하나**를 돌려준다. `activity.py:66` 은 `case_id` 를 넘기는데 **그 인자는 쓰이지 않는다.** Case 를 셋으로 나눠도 **셋이 같은 예약을 읽는다** |
...
★`[2026-09-10 정정]` **번역 성능 비교 자료(`datasets/mt/*`)는 제품 경로에서 빠진다** —
우리 출력이 한국어 하나라 번역이 우리 층에서 일어나지 않는다(§1). **개인 에이전트
연동 시험 자료로는 쓸 수 있다.** 초판은 "인바운드 영어 응대 검토에 바로 쓰인다"로
적었는데 언어 결정 전 문장이다.
...
| ★**언어 — 우리는 한국어만 낸다** | `[2026-09-10]` 1순위 고객이 **인바운드 외국인**인데 우리 출력은 한국어 하나다. 개인 에이전트가 옮긴다는 전제인데, **여행계획서 링크와 알림은 고객에게 직접 간다**(§1) — **그 경로에는 옮기는 주체가 없다** | ★**아직 안 정했다** → §0-3. 알림을 에이전트 경유로 보내거나, 직접 경로의 언어를 따로 정하거나, 인바운드 약속을 줄여야 한다 |
