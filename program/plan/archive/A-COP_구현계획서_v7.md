...
이 문서(v7.md)는 v6 전체를 계승한 새 기준선이며, 이후 구현·평가·심사의 기준선이다. v5·v6·`A-COP_구현계획서(4).md`·`A-COP_구현계획서_A2A_Graph반영.md`는 보존본이며 수정하지 않는다.
...
| 이후 기준선 | v7(이 파일)이 이후 구현·평가·심사의 기준선이다. v6와 이전 버전은 보존본이다. |
...
```text
   ...
   (10주 착수)                 Return(Mock) · Catalog(A2A Remote)
                              (10주 착수 일부 + 전면 자동화는 vision)
```
...
Team을 늘리는 일이 리팩토링이 되면 설계가 잘못된 것이다. [v7.1] 여기서 vision은 재고·가격·발주·배송·정산의 전면 운영 자동화다. 10주 착수 목록은 로컬의 Procurement + Order & Payment 통합, Fulfillment & Logistics, VOC & Store Manager, Return & Refund(Mock), Response Generation & Review와 A2A Remote의 Catalog & Verification이다. 검증 쇼핑몰을 돌리는 데 필요한 발주 실행은 §8-B의 착수 범위에 포함하며, 국외 배송·해외 구매대행의 실제 Live 연동은 Mock으로 남긴다.
...
필요 Team과 착수 Team을 구분한다. 기능상 필요 Team은 Catalog & Verification, Procurement, Order & Payment, Fulfillment & Logistics, Return & Refund, VOC/Feedback Analytics다. [v7.1] 10주 착수 구현의 LOCAL은 Procurement + Order & Payment 통합, Fulfillment & Logistics, VOC & Store Manager, Response Generation & Review다. A2A Remote는 Catalog & Verification으로 두고 별도 프로세스 fixture로 실제 왕복을 구현한다. Return & Refund는 Registry 계약 + Mock으로 유지한다. 단, 실제 스마트스토어 반품·교환 데이터에서 반품 사유 코드 체계와 상태 전이가 확인되고, 그 데이터로 골든셋 정답을 구성할 수 있음이 검증되면 LOCAL 착수로 승격한다. 데이터 확인 전에는 승격하지 않는다. **Registry 등록은 실제 구현을 의미하지 않는다.** 등록된 capability가 있어도 실행 가능한 module·fixture·contract test가 없으면 구현 완료로 세지 않는다.
...
[확인 필요] 권한 동기화 오류(계정 권한과 실제 부여 상태 불일치)를 판단하던 옛 Technical Entitlement Team은 §8-B의 착수 목록(Procurement + Order & Payment, Fulfillment & Logistics, VOC & Store Manager, Return & Refund, Response Generation & Review, Catalog & Verification)에 대응하는 팀이 없다. 이 업무를 어느 Team이 맡을지, 혹은 착수 범위에서 빠지는지 결정되지 않았다. 이름만 바꿔서 채우지 않는다.
...
착수 시점에는 Procurement + Order & Payment · Fulfillment & Logistics · VOC & Store Manager · Response Generation & Review 축으로 나누고, Catalog & Verification은 A2A Remote로 둔다.
...
| 2 | CAS·transition·Registry | REST/MCP skeleton | Procurement+Order, Fulfillment, VOC & Store Manager, Response contract, Catalog A2A Card | harness skeleton |
...
| 6 | Shared State merge | Catalog Verification 더미 A2A adapter | VOC 위임 제안·Response GEN/REV·`ActionProposal` 흐름 | end-to-end demo |
| 7 | GraphStorePort·SQL adapter | MCP/A2A 보안 | 관계 질의용 fixture | Graph gate 측정 |
| 8 | 재처리·경합 테스트 | input-required·callback·cancel | LOCAL/A2A 동일 결과, VOC·Response 평가 | 모듈별 golden/holdout 실행 |
