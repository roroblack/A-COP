...
인바운드 액티비티 예시: **골프(티타임)**, 한강 수상 액티비티, 겨울 스키·눈썰매, 템플스테이, 공연·뮤지컬, 테마파크, 쿠킹 클래스.

★**앞의 셋이 이 Team 의 규칙에 맞는 상품이다.** 이 Team 의 판정 규칙은 **날씨 조건**이고 감시 소스는 **기상청 초단기예보**인데(위 표), 뒤의 넷은 실내이거나 예약 시각이 고정이 아니어서 기상 감시가 의미가 없거나 약하다. 골프는 **우천 위약금 규정이 문서로 존재하고 티타임이 고정**이라 이 규칙에 가장 잘 맞는데 초판 목록에서 빠져 있었다(2026-09-09 추가).

★**이건 예시 고르기가 아니라 Team 경계 문제다.** 실내 티켓류를 Activity 에 넣으면 감시 소스가 둘로 갈린다(기상청 vs 공연 취소 공지) — §5 가 정한 "객체 종류별로 나눈다"와 충돌한다. 갈래 셋(정의를 넓혀 감시 소스를 둘로 · Activity 를 레저로 좁히고 티켓류를 분리 · 예시만 고침)의 비교는 `A-COP_여행Team모듈_구성안.md` §「Activity 예시」에 있다. `[미확보]` 인바운드에서 골프·스키가 티켓류보다 큰지는 안 봤다 — 그 수요 비교가 있어야 정의를 확정할 수 있다.
