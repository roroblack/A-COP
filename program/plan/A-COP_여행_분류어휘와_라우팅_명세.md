...
> ★`[2026-09-10]` **이 문서의 결정은 기준선 v11 에 올라갔다.**
> 범위·결정·일정은 [`A-COP_구현계획서_v11.md`](A-COP_구현계획서_v11.md) 가 정본이고,
> **여기 남은 것은 그 결정이 어떤 실측 위에서 나왔나** 다. 결정을 뒤집으려면 여기부터 읽는다.
> 흡수된 자리 — v11 §5-B(라우팅 두 축·어휘 값) · §5-C(capability 선택)
...
★`[결정 2026-09-10]` **위 다섯을 쓴다.** v10 §5-A 가 한국어만 적어 뒀으므로
슬러그는 이 문서가 정한다. 고른 기준 셋 —

| | |
|---|---|
| ASCII | 기존 `INTENTS` 도 그랬다. 값이 프롬프트·DB·평가셋을 오간다 |
| 뜻이 일대일 | `itinerary_submit` ↔ 일정 제출. 옮겨 읽을 여지가 없다 |
| 접두 충돌 없음 | `issue_code` 접두(객체 종류)와 겹치는 낱말이 없다 — 겹치면 라우팅에서 헷갈린다 |

**바꾸려면 v10 §5-A 에 슬러그를 적고 여기를 따라 고친다.** 두 곳에 다르게 적히면
그게 다음 드리프트다.
...
★`[결정 2026-09-10]` **같은 검사를 CI 와 기동 때 둘 다 돌린다.** 설정으로 빼는
대가가 "런타임 실패" 가 되면 안 된다. CI 가 배포 전에 잡고, 기동 검사는 손으로 고친
설정을 막는다. `[참고]` 어휘를 설정으로 빼면 검증 시점이 기동 때로 옮겨간다. 지금은 코드
...
★`[실측 2026-09-10]` **경로가 이미 있다.** `controller.py:171` 이
`except RegistryError` 로 잡아 `ROUTING_FAILED`(`failure_code: "no_team"`) 로
전이하고 `status: "escalated"` 를 돌려준다. `routing` 에 남지 않는다.
