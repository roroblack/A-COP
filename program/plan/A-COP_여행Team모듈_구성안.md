...
| Team — **뼈대를 이어받음** | **3** | **Activity**(레저 활동 정보·예약) · **Mobility**(구간 이동) · **Booking Handoff**(업체 예약 인계). 판정 구조를 각각 `return_refund` · `fulfillment_logistics` · `procurement_order_payment` 에서 가져온다 — **출처이지 팀 이름이 아니다** |
| Team — **새로 만듦** | **1** | Dining |
...
★**이 표는 「무엇을 베꼈나」를 적은 것이지 「무엇인가」를 적은 것이 아니다.** `return_refund` 줄이 Activity 를 환불 팀으로 만들지 않는다 — Activity 는 **골프·수상·스키 같은 레저 활동**을 맡아 **정보를 제공하고 예약 성립을** 판정하며, 그 판정이 취소 기한·위약금율을 다루기 때문에 구조가 같을 뿐이다.
...
|---|---|---|---|---|---|---|
| 1 | **Activity** | **레저 활동** — 골프(티타임) · 한강 수상 · 겨울 스키·눈썰매. **정보 제공 + 예약 판정** | 예약 시간·운영일·인원·날씨 조건·취소 기한·위약금율 | 운영 공지, 기상청 초단기예보, 예약 확인 | 같은 시간대 대체 액티비티, 날짜 이동, 환급 안내 | `return_refund` |
