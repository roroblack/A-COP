...
"크롤러가 카드마다 주문을 하나씩만 읽는다"가 아니다. 코드는 카드 안의 **모든**
상세 링크를 순회하고 있고(`for link_index in range(detail_links.count())`), 수집
당시 로그도 모든 페이지에서 **카드 수 = 주문 링크 수**였다. 화면이 그 카드에 상세
링크를 하나만 내준 것이다. 이 크롤러는 주문번호를 **상세 URL 에서만** 얻으므로
(`orders.pay.naver.com/order/status/(\d+)`), 자기 링크가 없는 주문은 식별할 방법이
없어 구조적으로 보이지 않는다.

★옛 진단대로 "링크 단위 순회로 바꾸기"를 하면 **이미 그렇게 된 코드를 다시
고치는 것**이라 한 세션을 헛되이 쓴다.

고치려면 로그인한 상태에서 주문이 둘 이상 든 카드의 DOM 을 실제로 떠 봐야 한다.
그 전에는 셀렉터를 바꿀 근거가 없다.

그동안 손실이 조용하지는 않게 해뒀다 — 상세 링크가 0개인 카드를 세어 완료 통계에
내고(전에는 아무 기록 없이 사라졌다), 모든 카드의 링크가 정확히 1개면 주의 문구를
남긴다. 자세한 수치는 `datasets/commerce/naver_order_history/REPORT.md`에 있다.
