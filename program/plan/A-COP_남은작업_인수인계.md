...
★**2026-09-01 갱신 — VOC 전처리는 끝났다.** 아래 표는 작성 당시(2026-08-24)
상태였고, 그 사이 다른 세션이 셋을, 이번에 나머지 셋을 처리했다.

| 데이터셋 | 원본 | 전처리 | 표본 |
|---|---|---|---:|
| `voc/aihub_71844_llm_instruction_tuning` | 209M | ✅ 2026-09-01 | 2,700 |
| `voc/aihub_102_smb_order_qa` | 189M | ✅ 2026-09-01 | 17,349 |
| `voc/nikl_ne_2022` | 137M | ✅ (다른 세션) | — |
| `voc/aihub_30716_callcenter_qa` | 83M | ✅ (다른 세션) | 1,500 |
| `voc/aihub_71603_aspect_sentiment` | 63M | ✅ 2026-09-01 | 17,283 |
| `voc/data_go_kr_consumer_complaints` | 284M | ✅ (다른 세션) | — |
| `voc/ecmc_dispute_casebook_2024` | 3.9M | ✅ (다른 세션) | — |
| `voc/naver_shopping_sentiment` | 20M | ✅ (다른 세션) | — |

각 폴더의 `REPORT.md` 에 무엇을 걸렀고 무엇을 못 옮겼는지 적혀 있다. 세 가지만
여기 옮겨 적는다 — 다음 세션이 이걸 모르고 쓰면 틀린 결론이 나온다:

- ★**`aihub_102` 의 감성 라벨은 쓰지 마라.** 96.8%가 중립이다(부정 3.1% ·
  긍정 0.08%). 이 분포로 학습하면 "전부 중립" 이라 답하는 모델이 정확도 96.8%를
  받는다. **감성은 `aihub_71603`**(긍정 60 · 중립 19 · 부정 13)을 쓴다
- ★**`aihub_102` 로는 반품/교환을 가를 수 없다.** 원본이 `교환|반품|환불` 을 한
  라벨로 묶어 뒀다(93,417건). `mapped_intent` 를 null 로 두고
  `ambiguous_between` 을 붙였다 — 찍으면 그건 지어낸 값이다
- ★**`aihub_71603` 의 SNS 35,616건은 인플루언서 홍보 글이다.** 쇼핑몰
  189,669건(실제 구매자 리뷰)과 뭉쳐 "고객 목소리" 라고 부르면 사실이 아니다
