...
판올림 날짜 2026-09-06. **절 번호와 DoD 번호는 그대로 둔다.** v8의 「v8 재판정」은 그대로 유효하다. 이번 판올림은 2026-09-06 사용자 결정 일곱을 계획서에 올리고, 계획서와 wiki의 역할을 나눈 것이다.
...
이 문서(v9.md)는 v8 전체를 계승한 새 기준선이며, 이후 구현·평가·심사의 기준선이다. v8 이하는 보존본(`program/plan/.archive/`, 2026-09-09 압축·숨김)이며 수정하지 않는다.
...
| 이후 기준선 | v9(이 파일)이 이후 구현·평가·심사의 기준선이다. v8 이하는 보존본이다. |
...
| 8-D | 구성 관리(Composer)와 설정 저장소의 소재 [v9 신설] | 두 배포 형태, 쓰기 계약, cs 소스 금지 |
...
바뀌지 않는다. 기업이 자기 Team을 꽂아 쓰는 플랫폼이다.
이 선택이 §8-B의 모듈화 설계와 §21의 계약 불변성을 상업적 근거로 만든다.

구축을 누가 하느냐는 이 성질과 별개다. 도입 기업이 직접 구성할 수도 있고 우리가 맡을 수도 있다.
어느 쪽이든 결과물은 플랫폼 위의 선언으로 남아 나중에 기업이 이어받을 수 있다.
제공 방식과 가격 정책은 아직 정하지 않았다. [2026-09-07 보강] 여전히 미정이지만 **안은 나와 있다** —
좌석당·건당·연간 라이선스 3안을 대 보고 연간 라이선스+규모 구간(C안)을 권하는 근거와 구간표가
`wiki/business/pricing.md`에 있다. 정하지 않은 것은 값이지 방향이 아니다. [2026-08-29 정정: 이전 판에는
"우리가 대신 만들어 주는 구축 서비스가 아니라"라는 문장이 있었으나 구축 대행을 배제할 의도가 아니었다.
제출표에도 그런 표현은 없다.]
...
[2026-09-07 정정] **위 결함 셋은 현재형으로 적혀 있으나 셋 다 해소됐다.** 이 문단은 2026-09-01에 발견한 것을 남긴 기록이므로 지우지 않고 결과만 덧붙인다. (1) 절차는 코어 1이 갖는다 — `app/application/classification.py::classify_case`가 실행 절차이고 접수 라우트(`app/presentation/api/cases.py`)는 "언제"만 정한다. 라우트에 절차가 박혀 있으면 Controller도 재시도 작업도 같은 분류를 부를 수 없다는 것이 고친 이유이며, 코드 주석이 이 결함 번호를 그대로 인용해 둔다. (2) 분류는 **생성 트랜잭션이 끝난 뒤**로 옮겼다. 커넥션을 쥔 것과 트랜잭션을 쥔 것은 다르므로 정적 검사로는 판정되지 않는다 — 실행 중 프로브로 **트랜잭션 IDLE · advisory lock 0개**를 확인했다(§0). (3) `require_module("voc", ...)` 호출은 **0곳**이다(`program/scripts/check_drift.py` 검사 3, 기대치 0). `voc: false`로도 기동한다. 아울러 `run_case` 호출처는 하나가 아니라 **셋**이다 — Controller 내부 resume, 접수 라우트(응답 뒤 `BackgroundTasks`로 떼어냄), `routing_sweeper`(되잡기). 접수는 더 이상 에이전트 실행을 기다리지 않는다. ★**방법은 여기 적힌 outbox가 아니라 `BackgroundTasks`였고 outbox 트리거는 만들지 않았다**(§25 4W). 위 실측 지연값 p50 20~34초·p95 32~51초는 **고치기 전 숫자**이며 재측정하지 않았다 `[미확보]`.
...
| 원인 축 판별·위임 대상 결정 | VOC Team | 실제 업무 판단. **다만 현재 구현이 없다** — `[실측 2026-09-07]` `app/modules/customer_ops/voc_store_manager.py`는 `llm`을 주입받고도 `execute()`에서 한 번도 쓰지 않는다. 실제 분기는 `similar_case_count >= 2` 고정 임계값 하나다 |
...
## 8-D. 구성 관리(Composer)와 설정 저장소의 소재 [v9 신설]

Composer는 운영자가 조립 선언(`config/project.yaml` — 어떤 Team·모듈을 켜고 끄는가)을 바꾸는 관리 기능이다. v8까지는 계획서 밖의 결정 문서들에만 있었다. 2026-09-06 사용자 결정으로 소재와 계약을 계획서에 올린다.

### 배포 형태 둘 — 둘 다 같은 패키지의 두 모드다

| 배포 형태 | Composer 위치 | cs와의 관계 | 릴리즈 때 |
|---|---|---|---|
| **pip 방식** (`direct`, **지금 기본**) | `acop_composer` 패키지 | 관리용 빌드의 cs가 pip로 설치해 라우터를 주입하고, UI는 그 cs의 `/composer/*`를 바라본다 | 패키지를 빼서 cs에서 제거된다 |
| **중앙 저장 방식** (`central`, 옵션) | UI 프로젝트 쪽 설정 서비스 한 곳 | cs는 자기 선언을 중앙에서 **읽기만** 한다 | cs에 Composer 코드가 애초에 없다 |
...
| 무엇 | 계약 |
|---|---|
| 운영자 화면 API | `/composer/toggle` · `/composer/changes`(create·update·delete·enable·disable) — **항목 하나 단위.** scope `composer:write` |
...
| 4W | 9/18~9/28, 11일 | 공식 신규 산출물 없음 | Context Broker·projection을 안정화한다. **[2026-09-07 정정] 여기 적었던 Controller 실행 경로 둘은 4W 전에 끝났다** — (가) Team 실행을 트랜잭션 밖으로(실행 중 프로브로 IDLE·lock 0 확인), (나) 접수의 동기 실행 해소(`_run_case_detached` + `background.add_task`, 틈은 `routing_sweeper`가 메움). 근거는 §0 「남은 것 둘」. ★**(나)는 여기 적었던 방법과 다르게 풀렸다.** 이 칸은 "outbox에 `case.created`를 넣고 worker가 Controller를 깨워" 풀라고 적었는데, 실제로는 `BackgroundTasks`로 떼어냈다. 결과(접수가 안 기다린다)는 같지만 **outbox 트리거는 만들지 않았다** — 안 적어 두면 나중에 그것을 찾게 된다. 아울러 §21 계약 변경도 필요 없었다. 접수 응답에 원래 `run_id`·`next_action`·`resume_token`을 넣지 않기로 했고(있다가 없다가 하는 필드는 클라이언트가 "없으면 실패"로 읽는다), 진행 결과는 `GET /v1/cases/{case_id}`로 확인한다. **4W는 Context Broker·projection 안정화만 남는다.** | Tool adapter·audit을 보완한다. | RAG 25/300~400과 Team 통합을 보완한다. | trace 화면과 발표 피드백을 반영한다. |
