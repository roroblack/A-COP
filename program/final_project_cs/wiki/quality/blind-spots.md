...
`[실측]` [DoD-19](../../../../final_project_cs/docs/evidence/DoD-19_LOCAL_A2A_정규화.md). `test_local_executor_is_identical_to_direct_team_call`이 두 실행 결과를 `model_dump()` 통째로 비교하는데, `observed_at=datetime.now(UTC)`가 두 호출에서 다르게 찍혀 **클럭 틱에 걸렸다.** 전체 실행 중 1회, 단독 실행은 통과 — 위 동시 실행 건과 겉모습이 같다.
