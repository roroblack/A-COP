...
**셋 — 검색이 한 번도 동작하지 않았던 기간이 있었는데 테스트는 초록이었다.** [DoD-06](../../../../final_project_cs/docs/evidence/DoD-06_정책FAQ_25건_300청크.md). 적재 직후 `search_policy()`는 **100% 실패**하고 있었다 — 질의 벡터가 tuple → `(...)`, list → `double precision[]`로 렌더링돼 `operator does not exist: vector <=> double precision[]`로 죽었다(`%s::vector` 캐스트로 해결).
