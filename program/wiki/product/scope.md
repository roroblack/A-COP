...
**1차 도메인은 온라인 커머스 고객운영이다.**

대표 시나리오 여섯.

```
주문 확인 · 배송 예외 · 반품 요청
반품 가능 여부 판정 · 환불 금액 산정 · 반복 불만 탐지
```

`[실측]` **골든셋 72건이 이 범위에서 구성돼 있다.** SaaS 구독 케이스는 0건이다.

### [2026-08-18 결정] 이전 도메인에서 옮겼다

이전 판의 1차 도메인은 **가상의 SaaS Customer Operations**였다 — 구독 해지 후 추가 결제, Free/Pro 권한 동기화, Seat·요금 불일치.

세 가지가 함께 바뀌었다.

| 무엇 | |
|---|---|
| Technical Entitlement Team | 착수 목록에서 뺐다 |
| 제품 구조 | CS Pack + Commerce Ops Pack으로 확정 |
| **도메인** | **커머스로 옮겼다** |

`final_project_sample`·`final_project_cs`에 남은 Billing/Technical 2종은 **Team 플러그인 아키텍처의 동작 증거(DoD-8)로만** 유지한다.

`[실측]` **v8 §6이 이 변경을 안 반영해 §10과 모순 상태다.** 이관 시 교체 대상이다.
