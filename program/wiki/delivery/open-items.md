...
| **cs `app/presentation/api/composer.py`의 `/apply`가 아직 `composer:write`** — D-011(통째 교체는 `composer:admin`, 이력·복원)이 sample에만 구현됐다. cs 에서 이 경로를 빼거나(D-006·D-CS-001) 같은 분리를 따라야 한다 | [D-011](../decisions/D-011-composer-v3-gap.md) |
...
| **3W 산출물 "학습한 ML/DL 모델"에 무엇을 내나** | 파인튜닝 미채택이라 LLM API만으론 안 채워진다 → [timeline.md](timeline.md) |
