# 작업 — final_project_sample 의 teams 문서 3건을 쓴다
...
```
final_project_sample/acop_basement/core/registry.py
final_project_sample/acop_basement/core/contracts.py
final_project_sample/acop_basement/teams/declarative.py
final_project_sample/tests/contract/test_team_contract.py
final_project_sample/tests/contract/test_core_isolation.py
final_project_sample/tests/contract/test_contracts.py
final_project_sample/tests/contract/test_module_toggles.py
final_project_sample/tests/integration/test_declarative_team_composition.py
```
...
| 파일 | 답해야 할 질문 |
|---|---|
| `team-contract.md` | Team 이 무엇을 받고 무엇을 돌려주나. 필수 필드가 무엇인가 |
| `team-boundary.md` | Team 이 하면 안 되는 것은 무엇이고 무엇이 그걸 막나 |
| `team-registry.md` | capability 로 Team 을 어떻게 찾나. 없으면 어떻게 되나 |

★`team-boundary.md` 는 `test_core_isolation.py` 가 정확히 무엇을 금지하는지를
  코드로 보여야 한다. "Core 는 Team 을 import 하지 않는다"는 선언이 아니라
  테스트가 어떻게 그걸 검사하는지를 적어라.
