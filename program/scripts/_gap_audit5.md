...
| 절 | 판정 | 어디에 / 무엇이 빠졌나 |
|---|---|---|
| 0. 문서 상태 | 일부 | `program/wiki/log.md`, `governance/drift-case-voc.md`; v8 기준선·이전 문서 우선순위·병합 출처 표가 온전히 없다 |
...
| 1. 모듈형 Basement — 4개 축 (§6) | 일부 | `program/wiki/architecture/core-design.md`, `system-context.md`, `core-vs-team.md`; Core Runtime·Agent Interoperability·Context & Knowledge·Agent Team Modules의 4축 표가 없다 |
...
| 4. ★신설해야 할 것 (현재 코드에 없음 — `git grep` 실측) | 일부 | `program/wiki/architecture/pack-model.md`, `decisions/D-002-graph-store-gate.md`, `program/final_project_cs/wiki/external/a2a-protocol.md`; 7개 컴포넌트의 당시 미구현 판정과 위치 표는 없다 |
| 5. 이동 규칙 — ★기존 동작을 깨지 않는다 | 일부 | `program/final_project_cs/wiki/runtime/`, `quality/invariants.md`; 113 passed 기준선, 기존 import re-export, 계층별 이동 순서가 빠졌다 |
| 6. 검증 | 일부 | `program/final_project_cs/wiki/operations/run.md`, `quality/evidence.md`; 원문의 두 명령과 “구조 이동으로 DoD가 바뀌면 동작 변경”이라는 판정이 한곳에 없다 |
...
| 절 | 판정 | 어디에 / 무엇이 빠졌나 |
|---|---|---|
| 0. 실행 환경 (실측 2026-08-12) | 반영 | `program/final_project_cs/wiki/data/schema/fields.md`, `operations/local-setup.md` |
| 1. DDL 전문 (v5 §8) | 반영 | `program/final_project_cs/wiki/data/schema/fields.md` |
| 2. ★빠뜨리면 시스템이 무너지는 제약 3개 | 반영 | `program/final_project_cs/wiki/data/schema/index.md`, `runtime/shared-state.md`, `actions/idempotency.md`, `actions/outbox.md` |
| 3. 쓰기 규칙 (전 스트림 필수) | 반영 | `program/final_project_cs/wiki/data/schema/index.md`, `runtime/case-lifecycle.md`, `data/tenancy.md` |
| optimistic concurrency (v5 §6-1) — 이 문 그대로 | 반영 | `program/final_project_cs/wiki/data/schema/fields.md` |
| outbox worker claim (v5 §6-4) | 반영 | `program/final_project_cs/wiki/data/schema/fields.md` |
| RAG 검색 (v5 §9-2) | 반영 | `program/final_project_cs/wiki/data/schema/fields.md`, `context/rag-retrieval.md` |
| 4. seed 데이터 (`scripts/seed.py`, S-DB 소유) | 반영 | `program/final_project_cs/wiki/data/schema/fields.md` |
| ★테스트는 seed 데이터를 오염시키지 않는다 | 일부 | `program/final_project_cs/wiki/data/schema/fields.md`; 전용 tenant·teardown 규칙은 있으나 오염으로 `tenants`가 19행이 된 실측이 없다 |
| 5. 검증 (DoD 근거로 `docs/evidence/` 에 남긴다) | 일부 | `program/final_project_cs/wiki/quality/evidence.md`, `operations/run.md`; `\dt` 14개+mock, `\di` 인덱스 3종+UNIQUE, `\dx` 2 extension의 검증 명령·합격선이 없다 |
...
| 절 | 판정 | 어디에 / 무엇이 빠졌나 |
|---|---|---|
| 0. 결론 먼저 | 일부 | `program/wiki/decisions/D-006-composer-ownership.md`, `program/final_project_cs/wiki/decisions/D-CS-003-composer-scope.md`; 당시 UI 패키지·CRUD·독립 배포 3건이 모두 미완이었다는 대조표가 없다 |
| 1. 먼저 — UI 작업을 이 세션에서 하는 게 맞나 | 일부 | `program/wiki/decisions/D-006-composer-ownership.md`, `architecture/repository-map.md`; 별도 저장소 규칙과 “sample은 패키지를 만들고 UI 화면은 UI 세션이 만든다”는 세션 경계가 완전하지 않다 |
| 2. 무엇이 빠져 있나 — UI 가 import 할 패키지 | 일부 | `program/wiki/decisions/D-006-composer-ownership.md`; 패키지 소유 결정은 있으나 당시 실제 패키지가 없었다는 판정이 없다 |
| 2.1 결정된 설계 | 일부 | `program/wiki/decisions/D-006-composer-ownership.md`; 3개 패키지 구조의 취지는 있으나 `acop_composer_ui`를 별도 세 번째 배포 단위로 둔 원문 구조와 다르다 |
...
| 4. ★인스턴스 CRUD 는 종단으로 성립하지 않는다 | 일부 | `program/final_project_cs/wiki/decisions/D-CS-003-composer-scope.md`; `/catalog` 필요성과 토글 한계는 있으나 UI grep 0건·화면 미연결 실측이 없다 |
| 5. pip 제공이 지금 구조로는 안 된다 | 누락 | 단일 `final-project-sample` 배포판이 `app`·`acop_basement`까지 포함한다는 `pyproject.toml` 판정이 없다 |
| 6. 지금 상태가 "틀린" 것인가 | 일부 | `program/wiki/decisions/D-006-composer-ownership.md`, `program/final_project_cs/wiki/decisions/D-CS-003-composer-scope.md`; 정상·미완·미연결·위험의 4행 판정표가 없다 |
| 7. 권고 — 다음 순서 | 일부 | `program/wiki/decisions/D-006-composer-ownership.md`, `delivery/open-items.md`; 권고 순서와 1~6번 작업 전체가 없다 |
| 7-1. sample 에서 할 일 (여기서 한다) | 일부 | `program/wiki/decisions/D-006-composer-ownership.md`; UI 클라이언트 경계는 있으나 독립 배포 메타데이터와 wheel 금지파일 검사가 없다 |
| 7-2. ui 세션에서 할 일 (거기서 한다) | 일부 | `program/final_project_cs/wiki/decisions/D-CS-003-composer-scope.md`; JSON Schema 폼·desired/effective 구분은 있으나 자체 클라이언트 교체와 화면 작업 순서가 없다 |
| 7-3. 하지 말 것 | 일부 | `program/wiki/decisions/D-006-composer-ownership.md`, `D-CS-003-composer-scope.md`; 서버 패키지 import·UI 판정 금지는 있으나 배포 분리 없이 pip 제공이라 쓰지 말라는 항목이 없다 |
| 8. 확인하지 못한 것 | 일부 | `program/wiki/delivery/open-items.md`, `decisions/D-011-composer-v3-gap.md`; 패키지 이름·cs 계약 문제는 있으나 HTTP 종단 미시험과 introspection 버전 불일치 재확인 항목이 없다 |
| 9. 조치 결과 (2026-08-29, 같은 날 후속) | 일부 | `program/wiki/decisions/D-007-central-config-store.md`; 구현 완료라는 상위 상태만 있고 §9-1~§9-6의 증거가 없다 |
...
| 9-7. 두 운영 방식 선택 (2026-08-30) | 일부 | `program/wiki/decisions/D-007-central-config-store.md`; direct/central 선택은 있으나 `X-Deployment-Id`, 화면의 현재 방식 표시, sample 443·UI 131 passed가 없다 |
| 9-5. 남은 것 | 일부 | `program/wiki/delivery/open-items.md`; 패키지 이름·cs 통합은 있으나 사설 index/릴리스 절차 미결정이 없다 |
...
| 절 | 판정 | 어디에 / 무엇이 빠졌나 |
|---|---|---|
| 한 줄 요약 | 반영 | `program/wiki/delivery/ticket-structure.md`, `delivery/timeline.md` |
...
| 절 | 판정 | 어디에 / 무엇이 빠졌나 |
|---|---|---|
| 0. 왜 하나 | 누락 | `program/final_project_cs/wiki/quality/blind-spots.md`는 현재 49/48/0 결과만 담고 원문의 39개·21검출·18생존·424통과 측정을 보존하지 않는다 |
...
| 1. 중앙 설정 저장소 (주말 최대 작업) | 일부 | `program/wiki/decisions/D-007-central-config-store.md`; 중앙 DB·direct/central 결정과 이유는 있으나 `ConfigStore`·`AuditStore`·마이그레이션 2개·통합 테스트 5개가 없다 |
| 2. Composer 카탈로그와 인스턴스 관리 | 일부 | `program/final_project_cs/wiki/decisions/D-CS-003-composer-scope.md`, `D-CS-004-composer-boundary.md`; 카탈로그·인스턴스 개념은 있으나 sample·UI·공용 클라이언트의 실제 위치와 이식 결과가 없다 |
| 3. DoD-28 파인튜닝 | 일부 | `program/wiki/evaluation/finetuning.md`, `program/final_project_cs/wiki/decisions/D-CS-002-finetuned-model-not-adopted.md`; 1,500건·376스텝·22건·0%는 있으나 holdout Proposed 16.7%, 손실 1.65→1.48, 정확도 0.67→0.71, 당시 16건 상한이 없다 |
| 4. 산출물 문서 | 누락 | 화면설계서·시스템 구성도·19장 중간발표 자료의 생성 방식과 주말 작업표가 없다 |
| 5. 일정 관리 | 일부 | `program/wiki/delivery/timeline.md`, `ticket-structure.md`; 4스프린트·13에픽 결정은 있으나 TeamFlow 권한 변화와 이슈 키 유지 이유가 없다 |
...
| 파인튜닝은 실패가 아니라 안 쓰기로 판정한 것이다 | 반영 | `program/final_project_cs/wiki/decisions/D-CS-002-finetuned-model-not-adopted.md` |
| 평가 수치는 아직 믿을 수 없다 | 반영 | `program/wiki/evaluation/metrics.md`, `delivery/release-gate.md` |
| 남은 일 | 일부 | `program/wiki/delivery/open-items.md`; 네이버 누락·VOC·평가·파인튜닝 항목 일부는 있으나 택배 제출본 병합과 VOC 924MB 등 원래 5행이 온전히 남지 않았다 |
...
| 절 | 판정 | 어디에 / 무엇이 빠졌나 |
|---|---|---|
| 0. 지금 상태 요약 | 일부 | `program/wiki/delivery/release-gate.md`, `delivery/dod.md`; RC 아님은 있으나 원문의 evidence 18/18·통과 16·부분 2·M1~M3 도달 수치가 없다 |
| 1. 빌드·테스트 | 일부 | `program/wiki/delivery/release-gate.md`, `program/final_project_cs/wiki/quality/evidence.md`; 실패·skip·flaky 0 기준은 있으나 172 passed·4회 연속·시계 의존 flaky 사례가 없다 |
| 2. 안전 (되돌릴 수 없는 것) | 반영 | `program/final_project_cs/wiki/runtime/case-lifecycle.md`, `actions/`, `external/mcp-tools.md`, `operations/unknown-state.md` |
| 3. 데이터·격리 | 반영 | `program/final_project_cs/wiki/data/tenancy.md`, `external/auth-boundary.md`, `quality/invariants.md` |
| 4. 배포 산출물 | 일부 | `program/wiki/delivery/release-gate.md`; `publish_public` 검사는 있으나 공개 183개·스크럽 21개·identity·워킹 트리 복구 수치와 결과가 없다 |
| 4-1. 배포 전 선언 변경 | 반영 | `program/final_project_cs/wiki/decisions/D-CS-001-composer-ui-removal.md` |
| 5. ★RC 를 막고 있는 것 | 반영 | `program/wiki/delivery/release-gate.md` |
| 5-1. judge agreement 미측정 (DoD-15) | 반영 | `program/wiki/delivery/release-gate.md`, `evaluation/judge.md` |
| 5-2. 평가 결과의 일반화 한계 (DoD-15 · v5 §15-8) | 반영 | `program/wiki/delivery/release-gate.md`, `evaluation/protocol.md` |
| 5-3. 미해결 (차단은 아님) | 일부 | `program/wiki/delivery/release-gate.md`에 같은 4항목이 있으나 모두 미해결로 적혀 있어 원문의 해소 결과·원인·14개 테스트·24장 캡처·E2E 5건이 빠지고 상태도 낡았다 |
| 6. 환경 (재현하려는 사람에게) | 반영 | `program/final_project_cs/wiki/operations/local-setup.md`, `operations/troubleshooting.md`, `program/wiki/architecture/tech-stack.md` |
| 7. 판정 | 반영 | `program/wiki/delivery/release-gate.md` |
