...
from acop_basement.core.contracts import InvalidTransition, StateConflict
    ...
    with get_connection() as conn:
        with conn.transaction():
            ...
            repository.create_action_request(conn, tenant_id=tenant, case_id=case_id, action_type="mcp.open_support_case",
                                            arguments={"request_id": request.request_id}, idempotency_key=idem)
        # ── 생성 트랜잭션이 끝났다 ──────────────────────────────────────────
        # ★계약(`CLAUDE.md` §0.2)은 이 tool 을 "Case 생성·**분류 시작**까지" 로
        #   정한다. 그런데 2026-09-06 이전에는 분류를 아예 시도하지 않고
        #   `classification_unavailable` 을 적어, MCP 로 연 Case 가 **전부 라벨
        #   없이 escalated** 로 갔다(실측 확인). 라우팅도 못 받는다.
        #   당시엔 이 경로에서 분류기를 구할 방법이 없어 정직하게 "못 한다" 고
        #   적은 것인데, 분류 절차가 코어 1로 올라오면서 그 이유가 없어졌다.
        #   ★REST 접수는 먼저 고쳤는데 이 경로가 남아 있었다 — 같은 일을 하는
        #   진입점이 둘이면 한쪽만 고쳐지는 날이 온다(cs 에서 먼저 발견).
