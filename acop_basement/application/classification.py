...
★2026-09-03 에 `final_project_cs` 에서 이식했다. 같은 결함이 여기에도 있었다 —
  cs 가 먼저 고쳤고 근거는 그 저장소의 `docs/reports/debugs/` 에 있다.
  ★평소 방향(sample → cs)과 **반대**다. 결함 수정은 먼저 찾은 쪽에서 온다.
...
★**왜 접수 라우트에서 뺐는가**(v8 §3-A [2026-09-01 교정] 결함 1). 전에는 이
  절차가 `acop_basement/presentation/api/cases.py` 안에만 있어서, 부를 수 있는 곳이 접수
