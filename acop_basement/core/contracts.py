...
class TeamManifest(BaseModel):
    ...
    # ★intent(거친 라벨)가 이 팀의 capability(세분화된 동작) 중 아무 것도
    #   이름으로 매칭하지 못할 때 쓸 capability 를 **팀이 직접 선언**한다.
    #   안 적으면 registry 가 `capabilities[0]` 을 쓰는데, 그 선택이 "왜 그건지"
    #   아무 데도 안 적힌 채 일어난다.
    #   cs 에서 먼저 만든 것을 이식했다(2026-09-06, cs
